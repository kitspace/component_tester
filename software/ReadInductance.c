...
void ReadInductance(void) {
  ...
  unsigned long
      inductance[4]; // four inductance values for different measurements
  union t_combi {
  ...
  uint8_t
      nr_pol1; // number of successfull inductance measurement with polarity 1
  uint8_t
      nr_pol2; // number of successfull inductance measurement with polarity 2
  union {
  ...
  for (count = 0; count < 4; count++) {
    ...
    if (count < 2) {
      ...
      rpins.pw =
          Rnum2pins(res_num); // compute the two pinnumbers for resistor res_num
    } else {
     ...
     ((PIN_RL3 + 1) != PIN_RH3))
    HiADC = pgm_read_byte((&PinRLRHADCtab[6]) + rpins.pb[1] -
                          TP_MIN); // Table of ADC Pins including | TXD_VAL
#else
    HiADC = pgm_read_byte((&PinRLRHADCtab[3]) + rpins.pb[1] -
                          TP_MIN); // Table of ADC Pins including | TXD_VAL
#endif
    LoPinR_L = pgm_read_byte(&PinRLRHADCtab[rpins.pb[0]] -
                             TP_MIN); // R_L mask for HighPin R_L load
    //==================================================================================
    ...
    if ((resistor < 240) && ((count & 0x01) == 0)) {
      ...
      ADC_DDR = HiADC | (1 << rpins.pb[0]) |
                TXD_MSK; // switch HiADC and Low Pin to GND,
    } else {
...
#define CNT_ZERO_720 7
    ...
    if (timeconstant.dw > cnt_diff)
      timeconstant.dw -= cnt_diff;
    else
      timeconstant.dw = 0;
    ...
    if (timeconstant.w[1] >= (F_CPU / 100000UL))
      timeconstant.dw = 0; // no transition found
...
/* ********************************************************* */
    ...
    if (((count & 0x01) == 0) &&
        (timeconstant.dw > ((F_CPU / 1000000UL) + 3))) {
      // transition is found, measurement with delayed counter start is not
      // necessary
      inductance[count + 1] =
          inductance[count]; // set delayed measurement to same value
  ...
  if (inductance[nr_pol2] < inductance[nr_pol1])
    nr_pol1 = nr_pol2;
