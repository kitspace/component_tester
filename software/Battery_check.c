...
// The divisor to get the voltage in 0.01V units is ((10*33)/133) witch is about
// 2.4812
...
#else
  bat_voltage =
      ((unsigned long)bat_adc * BAT_NUMERATOR) / BAT_DENOMINATOR + BAT_OUT;
#endif
#endif
#if FLASHEND > 0x1fff
  DC_Pwr_mode = 0;
...
#define WARN_LEVEL                                                             \
  (((unsigned long)(BAT_POOR + 800) * (unsigned long)BAT_DENOMINATOR) /        \
   BAT_NUMERATOR)
#elif (BAT_POOR > 3249)
// less than 5.4 V only .4V difference to Warn-Level
#define WARN_LEVEL                                                             \
  (((unsigned long)(BAT_POOR + 400) * (unsigned long)BAT_DENOMINATOR) /        \
   BAT_NUMERATOR)
#elif (BAT_POOR > 1299)
// less than 2.9 V only .2V difference to Warn-Level
#define WARN_LEVEL                                                             \
  (((unsigned long)(BAT_POOR + 200) * (unsigned long)BAT_DENOMINATOR) /        \
   BAT_NUMERATOR)
#else
// less than 1.3 V only .1V difference to Warn-Level
#define WARN_LEVEL                                                             \
  (((unsigned long)(BAT_POOR + 100) * (unsigned long)BAT_DENOMINATOR) /        \
   BAT_NUMERATOR)
#endif
#define POOR_LEVEL                                                             \
  (((unsigned long)(BAT_POOR) * (unsigned long)BAT_DENOMINATOR) / BAT_NUMERATOR)
