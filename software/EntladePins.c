...
void EntladePins() {
  uint8_t
      adc_gnd; // Mask of ADC-outputs, which can be directly connected to GND
...
#define MAX_ENTLADE_ZEIT (10000 / 20)
  ...
  for (lop_cnt = 0; lop_cnt < 10; lop_cnt++) {
    ...
    //     R_DDR = (1<<PIN_RH3) | (1<<PIN_RH2) | (1<<PIN_RH1); // R_H for all
    //     Pins to GND
    R_DDR = (1 << PIN_RH3) | (1 << PIN_RL3) | (1 << PIN_RH2) | (1 << PIN_RL2) |
            (1 << PIN_RH1) | (1 << PIN_RL1); // R_H and R_L for all Pins to GND
    ...
    uart_putc('T');
    u2lcd_space(
        adcmv[0]); // lcd_string(utoa(adcmv[0], outval, 10)); lcd_space();
    u2lcd_space(
        adcmv[1]);   // lcd_string(utoa(adcmv[1], outval, 10)); lcd_space();
    u2lcd(adcmv[2]); // lcd_string(utoa(adcmv[2], outval, 10));
    wait2s();
#endif
    if ((PartFound == PART_CELL) ||
        ((adcmv[0] < CAP_EMPTY_LEVEL) && (adcmv[1] < CAP_EMPTY_LEVEL) &&
    ...
    // all Pins with voltage lower than 1V can be connected directly to GND
    // (ADC-Port)
    if (adcmv[0] < 1000) {
    ...
    // since there is no disadvantage for the already directly switched pins, we
    // can
    // simply switch all  R_L resistors to GND
    //     R_DDR = (1<<PIN_RL3) | (1<<PIN_RL2) | (1<<PIN_RL1);	// Pins
    //     across R_L resistors to GND
    for (clr_cnt = 0; clr_cnt < MAX_ENTLADE_ZEIT; clr_cnt++) {
      ...
      if (adcmv[0] < 1300) {
        ADC_DDR |=
            (1 << TP1); // below 1.3V , switch directly with ADC-Port to GND
      }
      if (adcmv[1] < 1300) {
        ADC_DDR |=
            (1 << TP2); // below 1.3V, switch directly with ADC-Port to GND
      }
      if (adcmv[2] < 1300) {
        ADC_DDR |=
            (1 << TP3); // below 1.3V, switch directly with ADC-Port to GND
      }
      if ((adcmv[0] < (CAP_EMPTY_LEVEL + 2)) &&
          (adcmv[1] < (CAP_EMPTY_LEVEL + 2)) &&
          (adcmv[2] < (CAP_EMPTY_LEVEL + 2))) {
        break;
      }
    }
