...
void lcd_spaces(int8_t number); // clear number characters (number negative ->
                                // no chars cleared)
...
uint8_t lcd_save_position(void); // save the last Character position
                                 // (line,column) and returns the line
void lcd_restore_position(
    void); // restore the character position, saved with lcd_save_position()
void lcd_next_line(uint8_t xx); // y is incremented by 1
void lcd_next_line_wait(
    uint8_t xx); // y is incremented by 1 and call wait_for_key_5s_line2
