...
// assuming a datapoint of Vgs=v1, Id=i, tries to calculate Idss (i.e., Id at
// Vgs=0)
// v0 must be the Vgs at which Id=0
{
  ...
  for (;;) {
    ...
    d = v1 >> 8;
    if (d == 0)
      d = 1; // prevent infinite loop
    v1 += d; // increase v by 0.4 %;  unfortunately the compiler doesn't do this
             // very smartly, insists on creating a 16-bit temporary variable
             // for d
    d = i >> 8;
    i += d;
    ...
    // no Idss measurement if Idss exceeds 40 mA, the ATmega's maximum pin
    // current
    // note that this is actually quite safe, since by the time there's 40 mA
    // running, the Vgs will be 40mA * 20 ohm = 0.8 V, so quite far from 0, so
    // Id will be less than those 40 mA
    if (v1 > v0)
      return i; // V exceeds Vp, so we've reached Vgs=0 without Id exceeding 40
                // mA, so we can safely do the Idss measurement
  }
}
#endif
...
#warning "hFE measurement without common collector circuit"
  ...
  unsigned long
      c_hfe; // amplification factor for common Collector (Emitter follower)
#endif
  ...
  */
  wdt_reset();
  addr = &PinRLRHADCtab[LowPin -
                        TP_MIN]; // Address of combined RL / RH / ADC pin table
  LoPinRL = pgm_read_byte(addr); // instruction for LowPin R_L
#if FLASHEND > 0x1fff
#if (((PIN_RL1 + 1) != PIN_RH1) || ((PIN_RL2 + 1) != PIN_RH2) ||               \
     ((PIN_RL3 + 1) != PIN_RH3))
...
#endif
  LoADCp =
      pgm_read_byte(addr); // instruction for ADC Low-Pin, including | TXD_VAL
  ...
  TriADCp = pgm_read_byte(
      addr); // instruction for ADC Tristate-Pin, including | TXD_VAL
#endif
  ...
  HiADCp =
      pgm_read_byte(addr); // instruction for ADC High-Pin, including | TXD_VAL
...
#if (PROCESSOR_TYP != 8)
#define EXACT_OTR
// with option EXACT_OTR the vCE0 and vCEs is determined with common emitter
// circuit for
// PNP and NPN transistors. Without this option set the circuit is common
// emitter for PNP and
// common collector for NPN.
  ...
  R_DDR = LoPinRL;
  adc.lp_otr = W5msReadADC(
      LowPin); // read voltage of Low-Pin  , without Gate current (-)
  R_DDR = 0;
  ...
  if (adc.lp2 < adc.vCEs) {
    adc.lp_otr = W5msReadADC(
        LowPin); // read voltage of Low-Pin again, without Gate current (+)
    if ((adc.lp2 + 288) > adc.vCEs)
      goto checkDiode; // no significant change
    adc.vCEs = adc.lp2;
  } else {
    if ((adc.vCEs + 288) > adc.lp2)
      goto checkDiode; // no significant change
  }
#else
  ...
  adc.lp_otrh = W5msReadADC(LowPin); // read voltage of Low-Pin  , with
                                     // tri-state ping to plus via RH; prevents
                                     // accidental triggering of PUT
#endif
...
#endif
  if ((v_change_n < 288) && (v_change_p < 288))
    goto checkDiode; // no significant change
...
//     ChargePin10ms(TriPinRL,1);	 	//else: discharge for P-channel (Gate to
//     VCC)
...
//  }
  ...
  // both of NPN transistors (normal and inverse) are found, if ntrans.count ==
  // 2
  // both of PNP transistors (normal and inverse) are found, if ptrans.count ==
  // 2
  ...
  // Germanium transistors have also high collector current with higher
  // temperature.
  ...
  //  if((PartMode <= PART_MODE_P_JFET) && (adc.lp_otr > 455)  && (adc.vCEs >
  //  455))

  //  if((adc.lp_otr > 455)  && (adc.vCEs > 455))
  //  if((adc.vCEs > 115)  && ((adc.vCEs+100) > adc.lp_otr))
  if ((adc.vCEs > 115) && ((adc.vCEs + adc.vCEs + 20) >
                           adc.lp_otr)) { // there is more than 650uA current
                                          // without TristatePin current
...
// Test if N-JFET or if self-conducting N-MOSFET
...
#endif
    R_DDR = LoPinRL |
            TriPinRH; // switch R_H for Tristate-Pin (probably Gate) to GND
    ...
    adc.lp2 =
        W10msReadADC(LowPin); // measure voltage at the assumed Source again
#if DebugOut == 5
    lcd_testpin(LowPin);
    lcd_data('f');
    lcd_testpin(HighPin);
    ...
    mVOut(adc.lp_otr);
#endif
    // If it is a self-conducting MOSFET or JFET, then must be: adc.lp2 >
    // adc.lp1
    if (adc.lp2 > (adc.lp1 + 599)) {
      ...
      ntrans.uBE = adc.lp1;
      ntrans.gthvoltage =
          unsigned_diff(adc.lp1, adc.tp1); // voltage GS (Source - Gate)
      ntrans.current =
          (unsigned int)(((unsigned long)adc.lp1 * 10000) / RR680MI); // Id 1uA
#ifdef SHOW_ICE
      ...
      if ((PartMode & 0x0f) == PART_MODE_JFET) {
        uint16_t i16;
        // extrapolate the quadratic relationship between Id and Vgs, to
        // estimate Idss
        i16 = expand_FET_quadratic(ntrans.ice0, ntrans.gthvoltage,
                                   ntrans.current);
#if DebugOut == 5
        ...
        if (i16 != 0) {
          ...
          //               ADC_PORT = HiADCp;       // drain to Vcc, source to
          //               gnd, both without resistors
          ADC_DDR = HiADCm | LoADCp;
          adc.lp2 = W10msReadADC(LowPin); // measure voltage at the Source; this
                                          // is the voltage drop across the
                                          // pin's ~20 ohm internal resistance!
          ADC_DDR = TXD_MSK; // disconnect drain and source immediately after
                             // measurement, since quite a lot of current may
                             // flow
          // this is almost the Idss, since the gate-source voltage is almost 0
          // (only the voltage drop across that 20 ohm resistance)
          i16 = (unsigned int)(((unsigned long)adc.lp2 * 10000) /
                               pin_rmi); // Idss 1uA
#if DebugOut == 5
          DisplayValue(i16, -6, ' ', 3);
#endif
...
#ifdef SHOW_R_DS
      if ((PartMode & 0x0f) != PART_MODE_JFET) {
        // For depletion MOSFET we try to put the Gate to the same level as
        // Source.
        // The source level is higher than 0V because of the Port output
        // resistance and current.
        // When the Drain-Source resistance is low, we can get nearly the same
        // voltage increase
        // at the Gate with current from the 680 Ohm Port (RL).
        ADC_PORT = TXD_VAL;
        ADC_DDR = LoADCm | TriADCp; // Low-Pin and Tristate-Pin fix to GND
        //            R_DDR = TriPinRL | HiPinRL;	// L-Resistor High-Pin and
        //            Tristate-Pin to output
        R_DDR = HiPinRL; // L-Resistor High-Pin and Tristate-Pin to output
        R_PORT = TriPinRL |
                 HiPinRL; // switch R_L for High-Pin and Tristate-Pin to VCC
        ...
        ntrans.uBE = RR680PL * (unsigned long)unsigned_diff(adc.hp2, adc.lp2) /
                     adc.rhp; // DS resistance in 0.1 OHm
      }
#endif
#endif /* end SHOW_ICE */
#if DebugOut == 5
    ...
    // if it is a self-conducting P_MOSFET or P-JFET , then must be:  adc.hp1 >
    // adc.hp2
    if (adc.hp1 > (adc.hp2 + 599)) {
...
#endif
      ptrans.gthvoltage =
          unsigned_diff(adc.tp1, adc.hp1); // voltage GS (Gate - Source)
      ptrans.current =
          (unsigned int)(((unsigned long)vcc_diff(adc.hp1) * 10000) /
                         RR680PL); // Id 1uA
#ifdef SHOW_ICE
      //	Test for cutoff Voltage, idea from Pieter-Tjerk
      ADC_PORT = TXD_VAL; // direct outputs to GND
      ...
      R_PORT = TriPinRH | HiPinRH; // High-Pin and Tristate-Pin across R_H to
                                   // Vcc
      //        ptrans.ice0 = W10msReadADC(TristatePin);	//measure
      //        voltage at assumed Source
      //        ptrans.ice0 -= ReadADC(HighPin);
      ptrans.ice0 =
          unsigned_diff(W10msReadADC(TristatePin),
                        ReadADC(HighPin)); // measure voltage at assumed Source
#ifdef FET_Idss
#if DebugOut == 5
      lcd_line4();
      lcd_data('P');
      ...
      if ((PartMode & 0x0f) == PART_MODE_JFET) {
        ...
        // extrapolate the quadratic relationship between Id and Vgs, to
        // estimate Idss
        i16 = expand_FET_quadratic(ptrans.ice0, ptrans.gthvoltage,
                                   ptrans.current);
#if DebugOut == 5
        DisplayValue(i16, -6, ' ', 3);
#endif
        ...
        if (i16 != 0) {
          ...
          ADC_PORT =
              HiADCp; // drain to GND, source to VCC, both without resistors
          ADC_DDR = HiADCm | LoADCm;
          adc.hp3 = vcc_diff(W10msReadADC(HighPin)); // measure voltage at the
                                                     // Source; this is the
                                                     // voltage drop across the
                                                     // pin's ~20 ohm internal
                                                     // resistance!
          ADC_DDR = TXD_MSK; // disconnect drain and source immediately after
                             // measurement, since quite a lot of current may
                             // flow
          // this is almost the Idss, since the gate-source voltage is almost 0
          // (only the voltage drop across that 20 ohm resistance)
          i16 = (unsigned int)(((unsigned long)adc.hp3 * 10000) /
                               pin_rpl); // Idss 1uA
#if DebugOut == 5
          DisplayValue(i16, -6, ' ', 3);
#endif
      ...
      if ((PartMode & 0x0f) != PART_MODE_JFET) {
        ...
        ADC_DDR = HiADCm | TriADCp;  // switch High-Pin and Tristate-Pin to
                                    // output
        R_PORT = 0; // switch R-Ports to 0
        R_DDR = TriPinRL |
                HiPinRL; // L-Resistor High-Pin and Tristate-Pin to output
        adc.hp2 = W5msReadADC(HighPin); // measure the voltage at the Source
        adc.lp1 = ReadADC(LowPin);      // voltage at the Drain
        ptrans.uBE = RR680MI * (unsigned long)unsigned_diff(adc.hp2, adc.lp1) /
                     adc.lp1; // DS resistance in 0.1 OHm
      }                       /* end if ((PartMode&0x0f) != PART_MODE_JFET) */
#endif
#endif
...
// #############################################################
  ...
  if (adc.rhp > (100 + adc.lp_otr)) {
    ...
    lcd_data('H'); // PHcc
#endif
    c_hfe = ((unsigned long)tmp16 *
             (unsigned long)(((unsigned long)R_H_VAL * 10000) /
...
#endif
    c_hfe =
        ((tmp16 / ((RR680PL + 500) / 1000)) * (R_H_VAL / 500)) / (adc.tp1 / 5);
#endif
  } else {
    ...
    c_hfe =
        (unsigned long)((unsigned long)unsigned_diff(tmp16, adc.tp1) * 100) /
        adc.tp1;
  }
#endif
#if DebugOut == 5
...
// if the component has no connection between  HighPin and LowPin
    ...
    if (adc.lp1 > 3422) {
      ...
      //#if FLASHEND > 0x1fff
      //        R_DDR = LoPinRL | TriPinRL;	//switch R_L port for
      //        Tristate-Pin (Base) to output (GND)
      //        wait_about5ms();		// load gate capacitor
      //        R_DDR = LoPinRL | TriPinRH;	//switch R_H port for
      //        Tristate-Pin (Base) to output (GND)
      //        adc.lp1 = W5msReadADC(LowPin);	//measure voltage at
      //        LowPin (assumed Collector)
      //#else
      R_DDR =
          LoPinRL |
          TriPinRH; // switch R_H port for Tristate-Pin (Base) to output (GND)
      adc.lp1 =
          W10msReadADC(LowPin); // measure voltage at LowPin (assumed Collector)
      //#endif
      adc.tp2 = ReadADC(TristatePin); // measure voltage at TristatePin (Base)
      adc.hp2 = ReadADC(HighPin);     // measure voltage at HighPin (assumed
                                  // Emitter)

      if (adc.tp2 > 2000) {
#ifdef WITH_PUT
        // most likely it's a PNP transistor, but it might be a PUT
        // (Programmable Unijunction Transistor)
        // to check this, make Tri-Pin (assumed base) high again; PNP will stop
        // conducting, PUT won't
        R_PORT = TriPinRH;
        tmp16 = W10msReadADC(
            LowPin); // measure voltage at LowPin (assumed Collector)
...
// e_hfe with 1% resolution for optocoupler
        ...
        lcd_data('h'); // Phce
#endif
        e_hfe = ((tmp16 / ((RR680MI + 500) / 1000)) * (R_H_VAL / 500)) /
                (adc.tp2 / 5);
#endif
#if DebugOut == 5
        lcd_data('c');
        ...
        if ((ptrans.count == 0) || (e_hfe > ptrans.hfe)) {
          ...
          ptrans.current = (unsigned int)(((unsigned long)adc.lp1 * 10000) /
                                          RR680MI); // Ic 1uA
#endif
...
// c_hFE = (Emitter current - Base current) / Base current
...
#endif
        {
          ...
          ptrans.uBE = unsigned_diff(
              adc.hp1, adc.tp1); // Base Emitter Voltage common collector
#if FLASHEND > 0x1fff            /* at least ATmega16 */
          ptrans.current = (unsigned int)(((unsigned long)adc.rhp * 10000) /
                                          RR680PL); // Ie 1uA
        ...
        if (update_pins != 0) {
          ...
          ptrans.ice0 = (unsigned int)(((unsigned long)adc.lp_otr * 10000) /
                                       RR680MI); // ICE0 1uA
          ptrans.ices = (unsigned int)(((unsigned long)adc.vCEs * 10000) /
                                       RR680MI); // ICEs 1uA
        }
#endif
      ...
      if ((adc.lp_otr < 97) && (adc.lp1 > 2000)) {
        ...
        PartFound =
            PART_FET; // P-Kanal-MOSFET is found (Basis/Gate moves not to VCC)
        ...
        if (adc.hp2 > (adc.lp1 + 250)) {
          ...
          ptrans.uBE = RR680MI *
                       (unsigned long)unsigned_diff(adc.hp2, adc.lp1) /
                       adc.lp1; // DS resistance in 0.1 OHm
#endif
        ...
        for (ii = 0; ii < 11; ii++) {
          ...
          R_DDR =
              LoPinRL | TriPinRH; // switch R_H for Tristate-Pin (Basis) to GND
          while (!(ADC_PIN & PinMSK))
            ; // Wait, until the MOSFET switches and Drain moves to VCC
              // 1 is detected with more than 2.5V (up to 2.57V) with tests of
              // mega168 and mega328
          R_DDR = LoPinRL;
    ...
    if (adc.lp1 > (100 + adc.lp_otr)) {
      ...
      c_hfe = ((unsigned long)tmp16 *
               (unsigned long)(((unsigned long)R_H_VAL * 10000) /
                               (unsigned int)RR680MI)) /
              (unsigned int)adc.rtp;
#else
#if DebugOut == 5
      lcd_data('h'); // Nhcc
#endif
      c_hfe = ((tmp16 / ((RR680MI + 500) / 1000)) * (R_H_VAL / 500)) /
              (adc.tp2 / 5);
#endif
    } else {
      // Low-Pin=RL- HighPin=VCC
      ...
      // compute c_hfe with RL Base resistor
      c_hfe =
          (unsigned long)((unsigned long)unsigned_diff(tmp16, adc.rtp) * 100) /
          adc.rtp;
    }
#endif
#if DebugOut == 5
    ...
    R_DDR =
        TriPinRL | HiPinRL; // RL port for High-Pin and Tristate-Pin to output
    ...
    if (adc.hp1 < 4400) /* limit for opto-coupler with low hFE */
    {
      ...
      // Test auf Thyristor:
      // Gate discharge
      //      ChargePin10ms(TriPinRL,0);	//Tristate-Pin (Gate) across R_L 10ms
      //      to GND
      // TRIAC's can be triggered with gate and A1 (C) swapped. The current
      // remains after triggering
      // from gate to A2 (A) instead of A1 to A2. I have found that in this
      // state the current will be lower,
      ...
      adc.hp3 = W5msReadADC(
          HighPin); // read voltage at High-Pin (probably Anode) again
      ...
      adc.hp2 = W5msReadADC(
          HighPin); // measure voltage at the High-Pin (probably Anode) again
#if DebugOut == 5
      lcd_line4();
      lcd_data('y');
      lcd_space();
      ...
      if ((adc.hp3 < 1600) && (adc.hp2 > 4400)
          ...
          && ((adc.hp1 + 150) > adc.hp4)) {
        // if the holding current was switched off the thyristor must be
        // switched off too.
        // if Thyristor was still swiched on, if gate was switched off =>
        // Thyristor
        ...
        ntrans.gthvoltage =
            unsigned_diff(adc.hp1, adc.lp2); // Anode-Cathode Voltage
#endif
        // Test if Triac
        R_DDR = 0;
        R_PORT = 0;
        ...
        R_DDR = HiPinRL | TriPinRL; // switch R_L port for TristatePin (Gate) to
                                    // output (GND) => Triac should be triggered
        if (W5msReadADC(TristatePin) < 977) {
          ...
          lcd_space();
#endif
          goto saveNresult; // measure voltage at the Tristate-Pin (probably
                            // Gate) ;
          // if to low, abort
        }
        ...
        if (W5msReadADC(HighPin) < 733) {
          ...
          lcd_space();
#endif
          goto saveNresult; // component has no current without base current =>
                            // no Triac => abort
        }
      ...
      ADC_DDR = LoADCm; // Low-Pin to output 0V
#if FLASHEND > 0x1fff
      //      R_DDR = HiPinRL | TriPinRL;	//R_L port of Tristate-Pin (Basis) to
      //      output
      //      R_PORT = HiPinRL | TriPinRL;	//R_L port of Tristate-Pin (Basis)
      //      to VCC
...
//      }
      ...
      if (adc.rtp > 2557) { // Basis-voltage R_H is low enough
        PartFound =
            PART_TRANSISTOR; // NPN-Transistor is found (Base is near GND)
...
// e_hfe with 1% resolution for optocoupler
        ...
        lcd_data('h'); // NHce
#endif
        e_hfe = ((tmp16 / ((RR680PL + 500) / 1000)) * (R_H_VAL / 500)) /
                (adc.rtp / 5);
#endif
#if DebugOut == 5
        lcd_data('c');
...
#if FLASHEND > 0x1fff /* at least ATmega16 */
          ntrans.current = (unsigned int)(((unsigned long)adc.rhp * 10000) /
                                          RR680PL); // Ic 1uA
#endif
          update_pins = 1;
        }
...
// hFE = (Emitterstrom - Basisstrom) / Basisstrom
...
#if FLASHEND > 0x1fff /* at least ATmega16 */
          ntrans.current = (unsigned int)(((unsigned long)adc.lp1 * 10000) /
                                          RR680MI); // Ie 1uA
        ...
        if (update_pins != 0) {
          ...
          ntrans.ice0 = (unsigned int)(((unsigned long)adc.lp_otr * 10000) /
                                       RR680MI); // ICE0 0.01mA
          ntrans.ices = (unsigned int)(((unsigned long)adc.vCEs * 10000) /
                                       RR680PL); // ICEs 0.01mA
        }
#endif
      ...
      if ((adc.lp_otr < 97) && (adc.rhp > 3400)) {
        ...
        PartFound = PART_FET; // N-Kanal-MOSFET is found (Basis/Gate will Not be
                              // pulled down)
        ...
        if (adc.hp2 > (250 + adc.lp2)) {
          ...
          ntrans.uBE = RR680PL *
                       (unsigned long)unsigned_diff(adc.hp2, adc.lp2) /
                       adc.rhp; // DS resistance in 0.1 OHm
#endif
        ...
        for (ii = 0; ii < 11; ii++) {
          ...
          while ((ADC_PIN & PinMSK))
            ; // Wait, until the MOSFET switch and Drain moved to low
              // 0 is detected with input voltage of 2.12V to 2.24V (tested with
              // mega168 & mega328)
...
// current is too low for diode in flow direction, try resistor
  ...
  // Test if Diode
  ADC_PORT = TXD_VAL;
  R_DDR = HiPinRH; // enable resistor from HiPin to ground during the upcoming
                   // discharge, so if there's some leakage in the circuit
                   // (e.g., via protection diodes) this can't upset the
                   // discharge test
  for (ii = 0; ii < 200; ii++) {
...
/* but a parallel to a capacitor mounted diode can not be found. */
  ...
  R_DDR = LoPinRL;
  adc.hp1 = unsigned_diff(W5msReadADC(HighPin),
                          ReadADC(LowPin)); // GND--R_L--LP--|<--VCC
  // test for capacitor behaviour
  adc.lp_otr = unsigned_diff(W5msReadADC(HighPin),
                             ReadADC(LowPin)); // GND--R_L--LP--|<--VCC
#if DebugOut == 5
  lcd_data('C');                               // debug
  lcd_data('c');                               // debug
...
// move the resistor to the Low side again
#else
  /* There is not enough space to detect the protection diode for N-D-MOS
   * correctly. */
  R_DDR = HiPinRH; // switch R_H port for High-Pin output (VCC)
  R_PORT = HiPinRH;
  ChargePin10ms(TriPinRL, 1);     // discharge of P-Kanal-MOSFET gate
  ...
  if ((adc.hp1 > 150) && (adc.hp1 < 4640) && (adc.hp2 < adc.hp1) &&
      (adc.hp1 > (adc.hp3 + volt_dif)) && (adc.hp3 > tmp16)) {
    // voltage is above 0,15V and below 4,64V => Ok
    // hp2 >= hp1 is only possible with capacitor, not with a diode, hp2 is
    // measured with 470k
    // (adc.hp3 > adc.hp1/16) was OK for most cases, but not for gate of a TRIAC
    // (where adc.hp1/100 detect the diode)
    // for resistors the expected value is about adc.hp1/670, so adc.hp1/100
    // should also be OK to differ with resistor
    if (PartFound < PART_DIODE) {
      PartFound =
          PART_DIODE; // mark for diode only, if no other component is found
...
#ifdef DebugOut
void mVOut(uint16_t vv) { DisplayValue(vv, -3, ' ', 3); }
#endif
