...
void GetResistance(uint8_t HighPin, uint8_t LowPin) {
  /*
  Function for checking the resistance of a component with the following pin
  assignment
  parameters:
...
*/
  ...
  */
  wdt_reset();
  addr =
      &PinRLRHADCtab[LowPin -
                     TP_MIN]; // address of the combined RL / RH / ADC pin table
  ...
  resnum = (LowPin - TP_MIN + HighPin - TP_MIN -
            1); // resistor-Number 0:1 = 0, 0:2 = 1, 1:2 = 2
...
// U_SCALE can be set to 4 for better resolution of ReadADC result
  ...
  ADCconfig.U_AVCC =
      (adc_vcc_reference *
       U_SCALE); // scale to higher resolution, mV scale is not required
...
#define MAX_REPEAT (700 / (5 + R_ANZ_MESS / 8))
  ...
  for (ii = 0; ii < NumOfDiodes; ii++) {
    ...
    if ((diodes.Anode[ii] + diodes.Cathode[ii]) == (HighPin + LowPin))
      goto testend;
  }
#endif
  ADC_PORT = TXD_VAL;
...
// voltage break down isn't insufficient
  ...
  if (adc.hp2 < (4972 * U_SCALE)) {
    // voltage breaks down with low test current and it is not nearly shorted
    // => resistor
...
// measurement > 60MOhm too big resistance
      ...
      lirx1 = (unsigned long)((unsigned int)R_H_VAL) * (unsigned long)adc.hp2 /
              vcc_diff(adc.hp2);
      // lirx2 (measurement at LowPin)
      lirx2 = (unsigned long)((unsigned int)R_H_VAL) *
              (unsigned long)vcc_diff(adc.lp2) / adc.lp2;
#define U_INT_LIMIT                                                            \
  (990 * U_SCALE) // 1V switch limit in ReadADC for atmega family
...
#define FAKT_LOW 4 // resolution is about four times better
      ...
      if (adc.hp2 < U_INT_LIMIT) {
        lrx1 = (lirx1 * FAKT_LOW + lirx2) /
               (FAKT_LOW + 1); // weighted average of both R_H measurements
      } else if (adc.lp2 < U_INT_LIMIT) {
        lrx1 = (lirx2 * FAKT_LOW + lirx1) /
               (FAKT_LOW + 1); // weighted average of both R_H measurements
      } else
#endif
      {
      ...
      lirx1 = (unsigned long)RR680PL *
              (unsigned long)unsigned_diff(adc.hp1, adc.tp1) /
              vcc_diff(adc.hp1);
      if (adc.tp2 < adc.lp1) {
        adc.lp1 = adc.tp2; // diff negativ is illegal
      }
      // lirx2 (Measurement at LowPin)
      lirx2 = (unsigned long)RR680MI *
              (unsigned long)unsigned_diff(adc.tp2, adc.lp1) / adc.lp1;

#ifdef AUTOSCALE_ADC
      if (adc.hp1 < U_INT_LIMIT) {
        lrx1 = (lirx1 * FAKT_LOW + lirx2) /
               (FAKT_LOW + 1); // weighted average of both R_L measurements
    ...
    if (PartFound < PART_TRANSISTOR) {
      if (ResistorChecked[resnum] != 0) {
        ...
        lirx1 = (labs((long)lrx1 - (long)ResistorVal[resnum]) * 10) /
                (lrx1 + ResistorVal[resnum] + 100);
        if (lirx1 > 0) {
...
//              goto testend; // <10% mismatch
          ...
          if (PartFound < PART_DIODE) {
            PartFound =
                PART_RESISTOR; // only mark as resistor, if no other part found
          }
...
#if U_SCALE != 1
  ADCconfig.U_AVCC =
      adc_vcc_reference; // scale to higher resolution, mV scale is not required
