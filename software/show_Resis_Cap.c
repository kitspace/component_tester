...
// resistance and inductance measured through resonance, rather tight format to
// fit on 3 lines
// 1-RR-LL-3 12.34kO   (the ohm sign doesn't fit this way, but most likely,
// there's no 'k' sign since coils tend to not have that much resistance)
// 12.3uH if 22.0nF
// 334.5kHz Q=12.3
...
#if FLASHEND > 0x3fff
// EXTRASPACES contains any extra spaces needed to fill out the line if wider
// than 16 characters, i.e., LCD_LINE_LENGTH-16 spaces
...
// they include precisely enough extra space to start from the 7th position on
// the line
// if already k characters more have been written, just add k to the starting
// address
#if RMETER_WITH_L
// strings to fill out the topline with "[RL]" or "[ R]"
// they include precisely enough extra space to start from the 7th position on
...
// address
#ifdef RMETER_WITH_L
const unsigned char RL_METER_str[] MEM2_TEXT = {
    ' ', ' ', ' ', ' ', ' ', EXTRASPACES ' ', '[', 'R', 'L', ']', 0};
#else
const unsigned char RL_METER_str[] MEM2_TEXT = {
    ' ', ' ', ' ', ' ', ' ', EXTRASPACES ' ', ' ', '[', 'R', ']', 0};
#endif
#else
const unsigned char RESIS_str_R2[] MEM2_TEXT = {
    ' ', ' ', ' ', ' ', ' ', EXTRASPACES ' ', ' ', '[', 'R', ']', 0};
#endif
...
// pin1 and pin2 are resistor's pin numbers, but ResistorList[0] should also be
// correctly filled
// assumes resistance has already been measured, but will do inductance
// measurements as appropriate
// "how" flag tells how to show the results: if set [R] or [RL] will be shown in
// top right corner
{
#ifdef RMETER_WITH_L
  lcd_testpin(pin1);
  ...
  // draw first line: the pin numbers, RR and possibly LL symbol, and possibly
  // [R] or [RL]
  byte lclx0 = (lc_lx == 0);
...
// second line: measured R value (but that goes on first line if lc_lx!=0), and
// measured inductance, if applicable

#ifdef SamplingADC
  if (!lclx0) { /* inductance measured by sampling method */
    lcd_space();
    RvalOut(
        ResistorList[0]); // show Resistance, probably ESR, still on first line
  }
#endif
  ...
  if (!lclx0) {                           /* Frequency found */
    ...
    DisplayValue16(lc_cpar, -12, 'F', 2); // on 2-line dispaly show parallel
                                          // capacitance with only 2 digits to
                                          // make room for the '+' sign at the
                                          // end of the line
#else
...
#endif
  {
    ...
    if (inductor_lpre < -2) {
      ...
      DisplayValue(inductor_lx, inductor_lpre, 'H',
                   3); // output classic inductance
    }
  }
// third line: measured resonance frequency and Q, if applicable
  ...
  if (lc_fx) {
    ...
    //  #if LCD_LINES>2
    //              // make sure we clean the third line, but only if the
    //              display actually has a 3rd line
    //              lcd_next_line(0);
    //  #endif
    lcd_next_line(0);
    if (last_line_used == 0) {
  ...
  lcd_line2();
  inductor_lpre =
      -1; // prevent ESR measurement because Inductance is not tested
...
#endif
  {
    ...
    if (ResistorsFound != 0) {
      ...
      sampling_lc_calibrate(
          249); // try a short time to fetch new capacity value
#endif
#ifdef RMETER_WITH_L
...
#if (LCD_LINES > 2)
  present_res = present_esr; // show every time a well ESR value is detected,
                             // Vloss extra line
#else
#if FLASHEND > 0x3fff
...
//  lcd_clear();
  ...
  {
    init_parts(); // set all parts to nothing found
    //     cap.v_loss = 0;		// clear vloss  for low capacity values
    //     (<25pF)!
    ReadCapacity(TP3, TP1);
    PartFound = PART_CAPACITOR;
#ifdef SamplingADC
    if (cap.cpre == -12 && cap.cval < 100) {
      // if below 100 pF, try the alternative measuring method for small
      // capacitors
...
#if FLASHEND > 0x1fff
  if ((tt == 15) && (DC_Pwr_mode == 1))
    return (0); // when DC_Mode, next cycle start with 0
#else
  if (tt == 15)
    return (0); // when DC_Mode, next cycle start with 0
#endif
