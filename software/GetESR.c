 ...
 * measurement */
/* should be initiated before the next ADC-clock (8 us). One ADC takes 13 ADC
 * clock + 1 clock setup. */
/* The setting to sleep mode takes 10 clock tics, the wakeup takes about 24
 * clock tics, but 8us are 64 clock tics. */
/* I have found no reason, why a reset of the ADC clock divider should occur
 * during ESR measurement. */
...
#define StartADCwait()                                                         \
  ADCSRA = (1 << ADSC) | (1 << ADEN) | (1 << ADIF) |                           \
           AUTO_CLOCK_DIV;     /* Start conversion */                          \
...
#if F_CPU == 8000000UL
#define DelaySmallCap()                                                        \
  wait2us(); /* with only 4 us delay the voltage goes down before SH */        \
...
#if F_CPU == 8000000UL
#define DelaySmallCap()                                                        \
  wait5us(); /* with only 4 us delay the voltage goes down before SH */        \
...
uint16_t GetESR(uint8_t hipin, uint8_t lopin) {
  ...
  if (PartFound == PART_CAPACITOR) {
    ...
    if (cap_val_nF < (1800 / 10))
      return (0xffff); // capacity lower than 0.18 uF
#else
    if (cap_val_nF < (900 / 10))
      return (0xffff); // capacity lower than 90 nF
#endif
...
#ifdef ADC_Sleep_Mode
    StartADCmsk = (1 << ADEN) | (1 << ADIF) | (1 << ADIE) |
                  AUTO_CLOCK_DIV; /* enable ADC and Interrupt */
    ADCSRA = StartADCmsk;         /* enable ADC and Interrupt */
#else
    StartADCmsk = (1 << ADSC) | (1 << ADEN) | (1 << ADIF) |
                  AUTO_CLOCK_DIV; /* enable and start ADC */
#endif
  }
  ...
  LoPinR_L = pgm_read_byte(
      &PinRLRHADCtab[lopin - TP_MIN]); // R_L mask for LowPin R_L load
  HiPinR_L = pgm_read_byte(
      &PinRLRHADCtab[hipin - TP_MIN]); // R_L mask for HighPin R_L load

#if (PROCESSOR_TYP == 644) || (PROCESSOR_TYP == 1280)
  /* ATmega640/1280/2560 1.1V Reference with REFS0=0 */
  SelectLowPin = (lopin | (1 << REFS1) |
                  (0 << REFS0)); // switch ADC to LowPin, Internal Ref.
  SelectHighPin = (hipin | (1 << REFS1) |
                   (0 << REFS0)); // switch ADC to HighPin, Internal Ref.
#else
  SelectLowPin = (lopin | (1 << REFS1) |
                  (1 << REFS0)); // switch ADC to LowPin, Internal Ref.
  SelectHighPin = (hipin | (1 << REFS1) |
                   (1 << REFS0)); // switch ADC to HighPin, Internal Ref.
#endif
  ...
  // conversion.
  // Real ADC-conversion is started with the next ADC-Clock (125kHz) after
  // setting the ADSC bit.
  for (ii = 0; ii < MAX_CNT; ii++) {
    ...
    StartADCwait(); // ADCSRA = (1<<ADEN) | (1<<ADIF) | (1<<ADIE) |
                    // AUTO_CLOCK_DIV;
    ADCSRA = (1 << ADSC) | (1 << ADEN) | (1 << ADIF) |
             AUTO_CLOCK_DIV; // enable ADC and start with ADSC
    wait4us();
  ...
  if ((sumvolt[1] + sumvolt[3]) > sumvolt[0]) {
    sumvolt[2] = (sumvolt[1] + sumvolt[3]) -
                 sumvolt[0]; // difference HighPin - LowPin Voltage with current
  } else {
    sumvolt[2] = 0;
  }
  if (PartFound == PART_CAPACITOR) {
...
#endif
  esrvalue =
      (sumvolt[2] * 10 * (unsigned long)RRpinMI) / (sumvolt[0] + sumvolt[2]);
  esrvalue += esrvalue / 14; /* esrvalue + 7% */
  esr0 = (int8_t)eeprom_read_byte(
      &EE_ESR_ZEROtab[hipin - TP_MIN + lopin - TP_MIN]);
  if (esrvalue > esr0) {
    esrvalue -= esr0;
  } else {
#ifdef AUTO_CAL
...
#ifdef ADC_Sleep_Mode
  SMCR =
      (0 << SM0) | (0 << SE); /* clear ADC Noise Reduction and Sleep Enable */
#endif
  return (esrvalue);
#else
  return (0);
