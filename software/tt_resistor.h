...
extern unsigned long
    ResistorVal[3]; // Values of resistor 0:1, 0:2, 1:2, in units of 0.1 ohms
extern uint8_t ResistorList[3]; // list of the found resistor Numbers
extern uint8_t
    ResistorChecked[3];        // 2, if resistor is checked in both directions
...
extern uint16_t lc_cpar;    // value of parallel capacitor used for calculating
                            // inductance, in pF
extern uint16_t lc_cpartmp; // used to "exfiltrate" the raw capacitance value
                            // from ReadCapacitance()
extern const uint16_t lc_cpar_ee
    EEMEM; // place for lc_cpar as calibration constant in eeprom
#endif
