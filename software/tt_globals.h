...
#ifdef AUTO_CAL
const int8_t RefDiff EEMEM =
    REF_R_KORR; // correction of internal Reference Voltage
...
const int16_t ref_offset EEMEM = REF_C_KORR; // default correction of internal
                                             // reference voltage for capacity
                                             // measurement
// the zero offset for capacity measurement for all pin combinations
// LoPin:HiPin                        2:1    3:1    1:2
// marker  3:2                   1:3    2:3
const uint8_t c_zero_tab[] EEMEM = {C_NULL,
                                    C_NULL,
                                    C_NULL + TP2_CAP_OFFSET,
                                    C_NULL + 2,
                                    C_NULL + TP2_CAP_OFFSET,
                                    C_NULL,
                                    C_NULL}; // table of zero offsets
// if the marker position of c_zero_tab is not equal the first position, the
// calibration has not run before
#endif
#ifdef SamplingADC
const uint16_t c_zero_tab2_lo[] EEMEM = {
    C_NULL * 100, C_NULL * 100, C_NULL * 100, C_NULL * 100,
    C_NULL * 100, C_NULL * 100, C_NULL * 100}; // zero offsets for SamplingADC
                                               // capacitance measurement, in
                                               // 0.01 pF, lo voltage
const uint16_t c_zero_tab2_hi[] EEMEM = {
    C_NULL * 100, C_NULL * 100, C_NULL * 100, C_NULL * 100,
    C_NULL * 100, C_NULL * 100, C_NULL * 100}; // same, hi voltage
#endif
#ifdef WITH_MENU
const int8_t big_cap_corr EEMEM =
    C_H_KORR; // default correction for big capacity measurement
#if defined(WITH_FREQUENCY_DIVIDER) && !defined(NO_FREQ_COUNTER)
#ifndef FREQ_SCALER
#define FREQ_SCALER 0 // 1<<0 = 1
#endif
const uint8_t f_scaler EEMEM =
    FREQ_SCALER; // default scaler for frequency measurement
#endif
#endif

const uint8_t EE_ESR_ZEROtab[] EEMEM = {
    ESR_ZERO, ESR_ZERO, ESR_ZERO, ESR_ZERO}; // zero offset of ESR measurement
#ifdef WITH_ROTARY_SWITCH
//  const uint8_t EE_RotarySwitch EEMEM = 0;	// rotation switch is not
//  detected
#endif
#if ((LCD_ST_TYPE == 7565) || (LCD_ST_TYPE == 1306) ||                         \
     (LCD_ST_TYPE == 8812) || (LCD_ST_TYPE == 8814) || defined(LCD_DOGM))
const uint8_t EE_Volume_Value EEMEM =
    VOLUME_VALUE; // Volume Value for ST7565 controller
#endif
#ifdef LCD_CHANGE_COLOR
const uint8_t EE_BG_COLOR1 EEMEM =
    LCD_BG_COLOR & 0xff; // lower bits of background color
const uint8_t EE_BG_COLOR2 EEMEM =
    LCD_BG_COLOR >> 8; // higher bits of background color
const uint8_t EE_FG_COLOR1 EEMEM =
    LCD_FG_COLOR & 0xff; // lower bits of foreground color
const uint8_t EE_FG_COLOR2 EEMEM =
    LCD_FG_COLOR >> 8; // higher bits of foreground color
#endif
...
#endif
#ifdef SamplingADC
extern const uint16_t c_zero_tab2_lo[] EEMEM; // zero offsets for SamplingADC
                                              // capacitance measurement, in
                                              // 0.01 pF, lo voltage
extern const uint16_t c_zero_tab2_hi[] EEMEM; // same, hi voltage
#endif
#ifdef WITH_MENU
extern int8_t big_cap_corr
    EEMEM;                     // default correction for big capacity measurement
#if defined(WITH_FREQUENCY_DIVIDER) && !defined(NO_FREQ_COUNTER)
...
#if ((LCD_ST_TYPE == 7565) || (LCD_ST_TYPE == 1306) ||                         \
     (LCD_ST_TYPE == 8812) || (LCD_ST_TYPE == 8814) || defined(LCD_DOGM))
extern const uint8_t EE_Volume_Value
    EEMEM; // Volume Value for ST7565 controller
#endif
#ifdef LCD_CHANGE_COLOR
...
struct Switch_t {
  ...
  union {
    ...
    uint8_t
        R[4]; // mask to switch a Pin with R_L,  mask to switch a Pin with R_H
  } Pin;
};
...
typedef struct {
  ...
  unsigned int
      uBE; // B-E-voltage of the Transistor or RDS for E-MOS; Idss for JFET
...
#define ices gthvoltage
  // note: don't change the total size of the above fields, since the offsets of
  // the following 3 fields are hard-coded in PinLayout.S
...
COMMON unsigned int
    ref_mv_offs; // Reference-voltage  in mV units with eeprom offset for C
COMMON unsigned int
    adc_internal_reference; // internal reference voltage of ADC in mV units
COMMON unsigned int adc_vcc_reference; // reference voltage of ADC,if switched
                                       // to VCC in mV units
COMMON unsigned int
    RHmultip; // Multiplier for capacity measurement with R_H (470KOhm)
...
COMMON struct cap_t {
  // Attention! If you change this structure, you must also change defines in
  // GetESR.S !!!!
...
COMMON int16_t
    load_diff; // difference voltage of loaded capacitor and internal reference

COMMON uint8_t
    WithReference;        // Marker for found precision voltage reference = 1
...
#ifdef AUTO_CAL
COMMON uint8_t
    pin_combination; // coded Pin-combination  2:1,3:1,1:2,x:x,3:2,1:3,2:3
...
struct Rotary_t {
  uint8_t state[(ROT_MSK + 1)]; // coded state history of the rotatry switch,
                                // bit 0 == state of A-switch, bit 1 = state of
                                // B-switch
...
COMMON uint8_t
    rotary_switch_present; // is set to 1, if rotary switch movement is detected
...
COMMON const uint8_t EE_Volume_Value
    EEMEM; // Volume Value for ST7565 controller
#endif
#ifdef LCD_CHANGE_COLOR
COMMON union {
