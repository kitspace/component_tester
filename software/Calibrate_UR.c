...
// all measurements take  more than 170ms time
  ...
  if ((mv2500 > 2250) && (mv2500 < 2750)) {
    ...
    WithReference = 1;
    adc_vcc_reference =
        (unsigned long)((unsigned long)ADCconfig.U_AVCC * 2495) / mv2500;
  }
  ADCconfig.U_AVCC = adc_vcc_reference; // set ADC to normal VCC value
#endif
#ifdef WITH_AUTO_REF
  ...
  u680 = ((ADCconfig.U_AVCC * 3) - sum_rm -
          sum_rp); // three times the voltage at the 680 Ohm
  pin_rmi = (unsigned long)((unsigned long)sum_rm * (unsigned long)R_L_VAL) /
            (unsigned long)u680;
  //  adcmv[2] = pin_rm;	// for last output in row 2
  pin_rpl = (unsigned long)((unsigned long)sum_rp * (unsigned long)R_L_VAL) /
            (unsigned long)u680;
