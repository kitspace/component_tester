...
#define KEYPRESS_LENGTH_10ms                                                   \
  10 /* change frequency only with >100ms key press */
  uint8_t key_pressed;
  uint8_t significant; // number of signigficant bits to show the frequency (+16
                       // for option)
  ...
  message_key_released(
      F_GEN_str); // display f-Generator and wait for key released
  // OC1B is connected with 680 Ohm resistor to TP2 (middle test pin)
  TCCR1A = (0 << COM1B1) | (1 << COM1B0) | (0 << WGM11) |
           (0 << WGM10); // CTC mode, count to OCR1A
...
#endif
  {
    ...
    if (right) {
      long_key_char =
          '<'; // the frequency string will move to the left by long key press
    } else {
      if (significant < (16 + 6)) {
        long_key_char =
            '>'; // move the frequency string to the right with one leading zero
      } else {
        long_key_char =
            'R'; // i100kHz is the last position, Reset to 1 by long key press
      }
    }
    ...
    hperiod.dw = (F_CPU + wish_freq) /
                 (wish_freq * 2); // total divider for selected frequenx
    divider = 1;
    if (hperiod.w[1] >= 256) {
      // divide by 1024
      TCCR1B = (0 << WGM13) | (1 << WGM12) | (1 << CS12) | (0 << CS11) |
               (1 << CS10); // divide clock by 1024
      divider = 1024;
    } else if (hperiod.w[1] >= 64) {
      // divide by 256
      TCCR1B = (0 << WGM13) | (1 << WGM12) | (1 << CS12) | (0 << CS11) |
               (0 << CS10); // divide clock by 256
      divider = 256;
    } else if (hperiod.w[1] >= 8) {
      // divide by 64
      TCCR1B = (0 << WGM13) | (1 << WGM12) | (0 << CS12) | (1 << CS11) |
               (1 << CS10); // divide clock by 64
      divider = 64;
    } else if (hperiod.w[1] >= 1) {
      // divide by 8
      TCCR1B = (0 << WGM13) | (1 << WGM12) | (0 << CS12) | (1 << CS11) |
               (0 << CS10); // divide clock by 8
      divider = 8;
    } else {
      // no divide
      TCCR1B = (0 << WGM13) | (1 << WGM12) | (0 << CS12) | (0 << CS11) |
    ...
    OCR1A = (hperiod.dw / divider) -
            1; // the remainder divider  match to 16-bit counter
    if (OCR1A == 0)
      OCR1A = 1;
#if (LCD_LINES > 2)
...
//     lcd_clear_line();
    ...
    long int diff_freq;
    diff_freq =
        (((F_CPU / 2) / divider) * 100) / (OCR1A + 1) - (wish_freq * 100);
#if (LCD_LINES > 2)
...
#endif
    if (key_pressed >= 200)
      break;                // more than 2.0 seconds
    if (key_pressed > 80) { /* more than 0.8 seconds key press */
      // change of f_incre
      if (right) {
        // frequency string will move to the left, upper digit will move to the
        // next lower digit
      ...
      if (key_pressed > KEYPRESS_LENGTH_10ms)
        f_digit++; // longer key press select next frequency
    }
#ifdef WITH_ROTARY_SWITCH
    ...
    if (rotary.count >= 0) {
      ...
      f_digit += (Max_Digit + 1 +
                  rotary.count); // decrease the frequency by rotary.count
    }
#endif
