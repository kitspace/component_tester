...
#define MIN_SELECT_TIME                                                        \
  50 /* 50x10ms must be hold down to select function without a rotary switch   \
        */
#ifdef POWER_OFF
...
#else
#define MODE_LAST                                                              \
  MODE_SHOW /* without POWER_OFF, the SHOW function is the last */
#define MODE_OFF 66
#endif
...
#ifdef WITH_FREQUENCY_DIVIDER
  if (func_number == MODE_FSCALER)
    setFScaler(); // set scaler to 1,2,4,8,16,32,64,128,256,512
#endif
  if (func_number == MODE_FREQ)
    GetFrequency(0);
#endif
#if PROCESSOR_TYP == 644
  if (func_number == MODE_HFREQ)
    GetFrequency(1); // measure high frequency with 16:1 divider
  if (func_number == MODE_H_CRYSTAL)
    GetFrequency(5); // HF crystal input + 16:1 divider
  if (func_number == MODE_L_CRYSTAL)
    GetFrequency(6); // LF crystal input, 1:1 divider
#endif
...
#ifdef WITH_SELFTEST
  if (func_number == MODE_SELFTEST)
    AutoCheck(0x11); // Full selftest with calibration
#endif
  if (func_number == MODE_VEXT)
    show_vext();
#if ((LCD_ST_TYPE == 7565) || (LCD_ST_TYPE == 1306) ||                         \
     (LCD_ST_TYPE == 8812) || (LCD_ST_TYPE == 8814) || defined(LCD_DOGM))
  if (func_number == MODE_CONTRAST)
    set_contrast();
#endif
#ifdef LCD_CHANGE_COLOR
  if (func_number == MODE_SELECT_FG)
    select_color(0); // select foreground color
  if (func_number == MODE_SELECT_BG)
    select_color(1); // select background color
#endif
...
#endif
  {
    if (func_number > MODE_LAST)
      func_number -= (MODE_LAST + 1);
#if (LCD_LINES > 3)
    uint8_t mm;
#ifdef WITH_HARDWARE_SERIAL
    uart_newline(); // start of new measurement
    for (mm = 0; mm < LCD_LINE_LENGTH; mm++)
      uart_putc('=');
...
// func_number is not in page list
    ...
    if (ii > 0)
      ll = 0; // reset timer, operator present
    if (DC_Pwr_mode == 1)
      ll = 0;
#else
...
#ifdef WITH_ROTARY_SWITCH
    if (rotary.incre >= FAST_ROTATION)
      break; // to much rotation
#ifdef POWER_OFF
    if (rotary.count != 0)
      ll = 0; // someone is working, reset timer
#endif
    if (rotary.count >= 0) {
      func_number +=
          rotary.count; // function number is increased by rotary steps
    } else {
      func_number += (MODE_LAST + 1 +
                      rotary.count); // function is decreased by rotary steps
    }
#endif
    if (ii > 0)
...
void message2line(uint8_t number) {
  if (number > MODE_LAST)
    number -= (MODE_LAST + 1);
  if (number == MODE_TRANS)
    lcd_MEM2_string(TESTER_str);
#ifndef NO_FREQ_COUNTER
  if (number == MODE_FREQ)
    lcd_MEM2_string(FREQ_str);
#ifdef WITH_FREQUENCY_DIVIDER
  if (number == MODE_FSCALER)
    lcd_MEM2_string(FScaler_str);
#endif
#endif
#if PROCESSOR_TYP == 644
  if (number == MODE_HFREQ)
    lcd_MEM2_string(HFREQ_str);
  if (number == MODE_H_CRYSTAL)
    lcd_MEM2_string(H_CRYSTAL_str);
  if (number == MODE_L_CRYSTAL)
    lcd_MEM2_string(L_CRYSTAL_str);
#endif
  if (number == MODE_FGEN)
    lcd_MEM2_string(F_GEN_str);
  if (number == MODE_PWM)
    lcd_MEM2_string(PWM_10bit_str);
  if (number == MODE_ESR)
    lcd_MEM2_string(C_ESR_str);
  if (number == MODE_RESIS)
    lcd_MEM_string(RESIS_13_str);
  if (number == MODE_CAP13)
    lcd_MEM_string(CAP_13_str);
#ifdef WITH_ROTARY_CHECK
  if (number == MODE_ROTARY)
    lcd_MEM2_string(RotaryEncoder_str);
#endif
  if (number == MODE_BIG_CAP_CORR)
    lcd_MEM2_string(SetCapCorr_str);
#ifdef WITH_SELFTEST
  if (number == MODE_SELFTEST)
    lcd_MEM2_string(FULLCHECK_str);
#endif
#ifdef WITH_VEXT
  if (number == MODE_VEXT)
    lcd_MEM_string(VOLTAGE_str);
#endif
#if ((LCD_ST_TYPE == 7565) || (LCD_ST_TYPE == 8814) ||                         \
     (LCD_ST_TYPE == 8812) || (LCD_ST_TYPE == 1306) || defined(LCD_DOGM))

  if (number == MODE_CONTRAST)
    lcd_MEM_string(CONTRAST_str);
#endif
#ifdef LCD_CHANGE_COLOR
  if (number == MODE_SELECT_FG)
    lcd_MEM_string(FrontColor_str);
  if (number == MODE_SELECT_BG)
    lcd_MEM_string(BackColor_str);
#endif
  if (number == MODE_SHOW) {
...
/* ****************************************************************** */
/* show_C_ESR measures the capacity and ESR of a capacitor connected to TP1 and
 * TP3 */
/* ****************************************************************** */
void show_C_ESR() {
  uint8_t key_pressed;
...
#ifdef WITH_ROTARY_SWITCH
    if ((key_pressed != 0) || (rotary.incre > 3))
      break;
#else
    if (key_pressed != 0)
      break;
...
//     ADC_DDR = TXD_MSK;               //activate Software-UART
...
#ifdef POWER_OFF
#ifdef WITH_ROTARY_SWITCH
    if ((key_pressed > 0) || (rotary.incre > 0))
      times = 0; // reset the loop counter, operator is active
    if (rotary.incre > 5)
      break; // fast rotation ends voltage measurement
#else
    if (key_pressed > 0)
      times = 0; // reset the loop counter, operator is active
#endif
    ...
    if (key_pressed == 0)
      key_long_pressed = 0; // reset the key long pressed counter
    if (key_long_pressed > 4)
      break; // five seconds end the loop
#ifdef POWER_OFF
    times = Pwr_mode_check(times); // no time limit with DC_Pwr_mode
#endif
  }    /* end for times */
...
void do_10bit_PWM() {
  ...
  message_key_released(
      PWM_10bit_str); // display PWM-Generator and wait for key released
  // OC1B is connected with 680 Ohm resistor to TP2 (middle test pin)
  TCCR1A = (1 << COM1B1) | (0 << COM1B0) | (1 << WGM11) |
           (1 << WGM10); // fast PWM mode, mode 7: count to 10 bit
...
#if PROCESSOR_TYP == 644
  R_DDR = (1 << PIN_RL1) | (1 << PIN_RL2) |
          (1 << PIN_RL3); // set TP1, DDD4(TP2) and TP3 to output
#else
...
#endif
  {
    if (percent != old_perc) {
      ...
      lcd_data('/');
      lcd_space();
      DisplayValue16(((unsigned long)PWM_MAX_COUNT * SERVO_DIV) / MHZ_CPU, -6,
                     's', 3);
#else
      DisplayValue16((((unsigned long)pwm_flip * 1000) + (PWM_MAX_COUNT / 2)) /
                         PWM_MAX_COUNT,
                     -1, '%', 5);
#endif
      lcd_clear_line();
    ...
    if (rotary.count >= 0) {
      ...
      percent += ((SERVO_MAX - SERVO_MIN) +
                  rotary.count); // decrease the duty cycle by rotary.count
    }
#endif
    if (key_pressed > 50) {
...
void set_contrast(void) {
  ...
  message_key_released(
      CONTRAST_str); // display Contrast and wait for key released
...
#endif
  {
    ...
    lcd_command(ECMD_SET_CONTRAST |
                (contrast & 0x7f)); // set the contrast value
    lcd_command(
        CMD_SET_NORMAL_INSTRUCTION); // return to normal instruction mode
    ...
    lcd_command(
        CMD1_PowerControl |
        ((contrast >> 4) & 0x07)); // booster on,off / set contrast C5:C4
...
#ifdef WITH_ROTARY_SWITCH
    if ((key_pressed != 0) || (rotary.incre > 0))
      times = 0; // reset counter, operator is active
#else
    if (key_pressed != 0)
      times = 0; // reset counter, operator is active
#endif
...
#ifdef WITH_ROTARY_SWITCH
    if (rotary.incre > FAST_ROTATION)
      break; // fast rotation ends setting of contrast
    if (rotary.count >= 0) {
      contrast += rotary.count; // increase the contrast by rotary.count
    } else {
      contrast += (MAX_CONTRAST + 1 +
                   rotary.count); // decrease the contrast by rotary.count
    }
#endif
    if (key_pressed > 0) {
      if (key_pressed > 40) {
  ...
  eeprom_write_byte((uint8_t *)(&EE_Volume_Value),
                    (int8_t)contrast); // save contrast value
}
...
void select_color(uint8_t xcol) {
  ...
  if (xcol == 0) {
    message_key_released(
        FrontColor_str); // display FrontColor and wait for key released
    ...
    message_key_released(
        BackColor_str); // display BackColor and wait for key released
  ...
  //  eeprom_write_byte((uint8_t *)(&EE_Volume_Value), (int8_t)contrast);	//
  //  save contrast value
  if (xcol == 0) {
...
void set_big_cap_corr(void) {
  ...
  // set the contrast value
  message_key_released(
      SetCapCorr_str); // display Capacity correction and wait for key released
...
#ifdef WITH_ROTARY_SWITCH
    if (rotary.incre > FAST_ROTATION)
      break;              // fast rotation ends setting of korr
  ...
  eeprom_write_byte((uint8_t *)(&big_cap_corr),
                    (int8_t)korr); // save korr value
...
void setFScaler(void) {
  ...
  // set the contrast value
  message_key_released(
      FScaler_str); // display freq-scaler and wait for key released
...
#endif
  {
    ...
    if (korr > 128)
      korr = 9; // wrap around to 1<<korr = 512
    if (korr > 9)
      korr = 0; // wrap around to 1<<korr = 1
#ifdef POWER_OFF
    times = Pwr_mode_check(times); // no time limit with DC_Pwr_mode
#endif
  } /* end for times */
