...
// Big Capacities (>50µF) are measured with up to 500 load-pulses with the 680
// Ohm resistor.
...
// computed by
// interpolating the corresponding values of the table RLtab and multiply that
// with the number
// of load pulses (*10).
...
//	#if R_L_VAL == 3300
//	// resistor   330 Ohm               300   325   350   375   400   425
//450   475   500   525   550   575   600   625   650   675   700   725   750
//775   800   825   850   875   900   925   950   975  1000  1025  1050  1075
//1100  1125  1150  1175  1200  1225  1250  1275  1300  1325  1350  1375  1400
//mV
//	const uint16_t RLtab[] MEM_TEXT =
//{43680,40214,37242,34667,32414,30425,28657,27076,25652,24364,23192,22123,21142,20240,19407,18636,17920,17253,16630,16048,15501,14988,14505,14049,13619,13212,12826,12460,12112,11781,11466,11165,10878,10603,10341,10089,
//9848, 9617, 9395, 9181, 8976, 8778, 8588, 8404, 8227};
//	#else
//	// resistor   680 Ohm               300   325   350   375   400   425
//450   475   500   525   550   575   600   625   650   675   700   725   750
//775   800   825   850   875   900   925   950   975  1000  1025  1050  1075
//1100  1125  1150  1175  1200  1225  1250  1275  1300  1325  1350  1375  1400
//mV
//	const uint16_t RLtab[] MEM_TEXT =
//{22447,20665,19138,17815,16657,15635,14727,13914,13182,12520,11918,11369,10865,10401,
//9973, 9577, 9209, 8866, 8546, 8247, 7966, 7702, 7454, 7220, 6999, 6789, 6591,
//6403, 6224, 6054, 5892, 5738, 5590, 5449, 5314, 5185, 5061, 4942, 4828, 4718,
...
const uint16_t LogTab[] PROGMEM = {0,    20,   41,   62,   83,   105,  128,
                                   151,  174,  198,  223,  248,  274,  301,
                                   329,  357,  386,  416,  446,  478,  511,
                                   545,  580,  616,  654,  693,  734,  777,
                                   821,  868,  916,  968,  1022, 1079, 1139,
                                   1204, 1273, 1347, 1427, 1514, 1609, 1715,
                                   1833, 1966, 2120, 2303, 2526, 2813, 3219,
                                   3912, 7065}; /* last value is replaced by
                                                   (-1000*ln(0.001)-3912)/19*20
                                                   + 3912 */
...
/*
 #if R_H_VAL == 10000
// resistor  100000 Ohm             1000 1050 1100 1150 1200 1250 1300 1350 1400
mV
 const uint16_t RHtab[] PROGMEM =
{4483,4244,4026,3827,3645,3477,3322,3178,3045};
 #else
// resistor  470000 Ohm            1000 1050 1100 1150 1200 1250 1300 1350 1400
mV
 const uint16_t RHtab[] PROGMEM = { 954, 903, 856, 814, 775, 740, 707, 676,
648};
 #endif
*/
...
// these values are 1e7/R_H_VAL/-ln(1-ref/5000)   (note that R_H_VAL is in
// multiples of 10 ohm)
...
const unsigned char RELPROBE[] MEM_TEXT =
    "isolate Probes!"; // or "separate pins!";
...
#endif
const unsigned char SetCapCorr_str[] MEM2_TEXT = {
    'C', '(', LCD_CHAR_U, 'F', ')', '-', 'c', 'o', 'r',
    'r', 'e', 'c',        't', 'i', 'o', 'n', 0};
...
const unsigned char L_CRYSTAL_str[] MEM2_TEXT = "LF quartz";
     ...
     (LCD_ST_TYPE == 8812) || (LCD_ST_TYPE == 8814) || defined(LCD_DOGM))
...
#if ((!defined(WITH_GRAPHICS) && (LCD_LINE_LENGTH > 17)) ||                    \
     (defined(WITH_GRAPHICS) && (LOW_H_SPACE != 1)))
...
const unsigned char N123_str[] MEM_TEXT = {
    ' ', '1' + TP_OFFSET, '2' + TP_OFFSET, '3' + TP_OFFSET, '=', 0};
// const unsigned char N123_str[] MEM_TEXT = " Pin=";
#else
#if EBC_STYLE == 321
const unsigned char N321_str[] MEM_TEXT = {
    ' ', '3' + TP_OFFSET, '2' + TP_OFFSET, '1' + TP_OFFSET, '=', 0};
#endif
#if EBC_STYLE == 123
const unsigned char N123_str[] MEM_TEXT = {
    ' ', '1' + TP_OFFSET, '2' + TP_OFFSET, '3' + TP_OFFSET, '=', 0};
...
const unsigned char CapZeich[] MEM_TEXT = {LCD_CHAR_LINE1, LCD_CHAR_CAP,
                                           LCD_CHAR_LINE1, 0};
...
const unsigned char HelpCalibration_str[] PROGMEM = {
    'N', 'o', 't', ' ', 'c', 'a', 'l', 'i', 'b', 'r', 'a', 't', 'e', 'd', '!'
#ifndef SHORT_UNCAL_MSG
    ,
    ' ', 'F', 'o', 'r', LCD_CHAR_INSEP, 'c', 'a', 'l', 'i', 'b', 'r', 'a', 't',
    'i', 'o', 'n', ' ',

    'c', 'l', 'a', 'm', 'p', ' ', 't', 'h', 'e', ' ', '3', LCD_CHAR_INSEP, 'p',
    'i', 'n', 's', ' ', 't', 'o', 'g', 'e', 't', 'h', 'e', 'r', ' ', 'a', 'n',
    'd', ' ',

    's', 't', 'a', 'r', 't', ' ', 'w', 'i', 't', 'h', ' ', 't', 'h', 'e',
    LCD_CHAR_INSEP, 'k', 'e', 'y', '.', ' ',

    'A', 'c', 'k', 'n', 'o', 'w', 'l', 'e', 'd', 'g', 'e', ' ', 't', 'h', 'e',
    ' ', 's', 'e', 'l', 'f', '-', 't', 'e', 's', 't', ' ', 'w', 'i', 't', 'h',
    ' ',

    't', 'h', 'e', ' ', 'k', 'e', 'y', ' ', 'w', 'i', 't', 'h', 'i', 'n',
    LCD_CHAR_INSEP, '2', 's', '.', ' ',

    'D', 'i', 's', 'c', 'o', 'n', 'n', 'e', 'c', 't', ' ', 'p', 'i', 'n', 's',
    ' ', 'a', 'f', 't', 'e', 'r', ' ',
    ...
    '.', ' ',
#else
    ' ', 'a', 'n', 'd', ' ', 'c', 'o', 'n', 'n', 'e', 'c', 't', ' ', 'a', ' ',
    'g', 'o', 'o', 'd', ' ',

    'c', 'a', 'p', 'a', 'c', 'i', 't', 'o', 'r', ' ', 'w', 'i', 't', 'h', ' ',
    'a', 't', ' ', 'l', 'e', 'a', 's', 't', ' ', '1', '0', '0', 'n', 'F', ' ',

    't', 'o', ' ', 'p', 'i', 'n', LCD_CHAR_INSEP, '1', LCD_CHAR_INSEP, 'a', 'n',
    'd', LCD_CHAR_INSEP, '3', ' ', 'o', 'n', 'l', 'y', ' ', 'a', 'f', 't', 'e',
    'r', ' ',

    't', 'h', 'e', ' ', 'm', 'e', 's', 's', 'a', 'g', 'e', ' ', '"',
    '1' + TP_OFFSET, LCD_CHAR_LINE1, LCD_CHAR_CAP, LCD_CHAR_LINE1,
    '3' + TP_OFFSET, LCD_CHAR_INSEP, '>', '1', '0', '0', 'n', 'F', '"', ' ',

    'i', 's', LCD_CHAR_INSEP, 's', 'h', 'o', 'w', 'n', '.', ' ',
#endif
    'S', 'o', 'f', 't', 'w', 'a', 'r', 'e', ' ', 'a', 'n', 'd', ' ',

    'd', 'o', 'c', 'u', 'm', 'e', 'n', 't', 'a', 't', 'i', 'o', 'n', ' ', 'i',
    's', ' ', 'a', 't', ' ', 's', 'v', 'n', ':', '/', '/', ' ',
...
const unsigned char AnKat_str[] MEM_TEXT = {LCD_CHAR_LINE1, LCD_CHAR_DIODE1,
                                            LCD_CHAR_LINE1, 0};
const unsigned char KatAn_str[] MEM_TEXT = {LCD_CHAR_LINE1, LCD_CHAR_DIODE2,
                                            LCD_CHAR_LINE1, 0};
#ifdef SamplingADC
...
const unsigned char Resistor_str[] MEM_TEXT = {
    LCD_CHAR_LINE1, LCD_CHAR_RESIS1, LCD_CHAR_RESIS2, LCD_CHAR_LINE1, 0};
#if FLASHEND > 0x1fff
const unsigned char Inductor_str[] MEM_TEXT = {
    LCD_CHAR_LINE1, LCD_CHAR_INDUCTOR1, LCD_CHAR_INDUCTOR2, LCD_CHAR_LINE1, 0};
#endif
...
#if FLASHEND > 0x3fff
#ifdef RMETER_WITH_L
const unsigned char RESIS_13_str[] MEM2_TEXT = {'1' + TP1,
                                                LCD_CHAR_LINE1,
                                                LCD_CHAR_RESIS1,
                                                LCD_CHAR_RESIS2,
                                                LCD_CHAR_LINE1,
...
const unsigned char CAP_13_str[] MEM2_TEXT = {
    '1' + TP1, LCD_CHAR_LINE1, LCD_CHAR_CAP, LCD_CHAR_LINE1, '1' + TP3, ' ', 0};
...
const unsigned char DiodeIcon1[] MEM_TEXT = {
    0x11, 0x19, 0x1d, 0x1f, 0x1d, 0x19, 0x11, 0x00}; // Diode-Icon Anode left

const unsigned char DiodeIcon2[] MEM_TEXT = {
    0x11, 0x13, 0x17, 0x1f, 0x17, 0x13, 0x11, 0x00}; // Diode-Icon Anode right
const unsigned char ResIcon1[] MEM_TEXT = {0x00, 0x0f, 0x08, 0x18, 0x08,
                                           0x0f, 0x00, 0x00}; // Resistor Icon1
const unsigned char ResIcon2[] MEM_TEXT = {0x00, 0x1e, 0x02, 0x03, 0x02,
                                           0x1e, 0x00, 0x00}; // Resistor Icon2
#if LCD_CHAR_RESIS3 != 'R'
const unsigned char ResIcon3[] MEM_TEXT = {0x04, 0x1f, 0x11, 0x11, 0x11,
                                           0x11, 0x1f, 0x04}; // Resistor Icon2
#endif

// generate Omega- and µ-character as Custom-character, if these characters has
// a number of loadable type
#if LCD_CHAR_OMEGA < 8
const unsigned char CyrillicOmegaIcon[] MEM_TEXT = {0,  0,  14, 17,
                                                    17, 10, 27, 0}; // Omega
#endif
#if LCD_CHAR_U < 8
const unsigned char CyrillicMuIcon[] MEM_TEXT = {0,  17, 17, 17,
                                                 19, 29, 16, 16}; //µ
#endif
...
const unsigned char PinRLRHADCtab[] PROGMEM = {
    ...
    (1 << PIN_RL3), // Table of commands to switch the  R-L resistors Pin 0,1,2
     ...
     ((PIN_RL3 + 1) != PIN_RH3))
    // Processors with little memory must use one Pin number higher than
    // correspondig Low Resistor
    /* PinRHtab */ (1 << PIN_RH1), (1 << PIN_RH2),
    (1 << PIN_RH3), // Table of commands to switch the  R-L resistors Pin 0,1,2
#endif
    // Table include the predefined value TXD_VAL of other output port(s) of
    // port C.
    // Every pin, that should be switched permanent to VCC level, should be set
    // to 1 in every tab position.
    // The predefined value TXD_MSK defines the pin (all pins), that must be
    // switched permanent to output.
    /* PinADCtab */ (1 << TP1) | TXD_VAL, (1 << TP2) | TXD_VAL,
    (1 << TP3) |
        TXD_VAL}; // Table of commands to switch the ADC-Pins 0,1,2 to output

const uint8_t PrefixTab[] MEM_TEXT = {'f', 'p', 'n', LCD_CHAR_U,
                                      'm', 0,   'k', 'M'}; // f,p,n,u,m,-,k,M
...
#ifdef SamplingADC
const unsigned char cap_for_l_meas_str[] MEM2_TEXT = {
    '1',          LCD_CHAR_LINE1,
    LCD_CHAR_CAP, LCD_CHAR_LINE1,
    '3',          ' ',
    '4',          '-',
    '3',          '0',
    'n',          'F',
...
#define ADref1V1                                                               \
  ((0 << REFS0) | (1 << REFS1)) // use built-in reference, about 1.1 V;
#else
#define ADref1V1                                                               \
  ((1 << REFS0) | (1 << REFS1)) // use built-in reference, about 1.1 V;
#endif
