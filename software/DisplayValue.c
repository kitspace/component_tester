...
void DisplayValue(signed long signedValue, int8_t Exponent, unsigned char Unit,
                  uint8_t xdigits) {
  ...
  if (digits == 3)
    Limit = 1000; /* scale value down to 3 digits */
  if (digits == 4)
    Limit = 10000; /* scale value down to 4 digits */
#ifdef LONG_OUTPUT
  if (digits == 5)
    Limit = 100000; /* scale value down to 5 digits */
  if (digits == 6)
    Limit = 1000000; /* scale value down to 6 digits */
  if (digits == 7)
    Limit = 10000000; /* scale value down to 7 digits */
#endif

  while (Value >= Limit) {
    Value += 5; /* for automatic rounding */
    // perhaps TODO: the above is not really correct, can do double rounding
    // (2000.45 -> 2000.5 -> 2001)
 ...
 */
  ...
  // also removes the limits checking, since limit errors should never happen,
  // and if they do, they'd show _incorrect_ results;
  // showing a garbage prefix character seems preferable to something
  // correct-looking but off by some power of 10
...
/* convert value into string */
...
#ifdef LONG_OUTPUT
  ultoa(Value, &OutBuffer[6],
        10); // sprintf(&OutBuffer[6],"%ld%c",Value,(char)0);
#else
  utoa((uint16_t)Value, &OutBuffer[6],
       10); // sprintf(&OutBuffer[6],"%d%c",Value,(char)0);
#endif
...
#ifdef NO_NANO
  if (Index == 2) { /* use no nano, prefix 'f','p','n','u','m',' ','k','M' */
    Index++;        /* use mikro instead of nano */
    Offset += 3;    /* can be 3,4 or 5 */
  }
#endif

  Prefix = MEM_read_byte(
      (unsigned char *)(&PrefixTab[Index])); /* look up prefix in table */
  ...
  if (Offset == 0)
    Exponent = -8; /* disable dot if not needed */
...
void DisplayValue16(uint16_t Value, int8_t Exponent, unsigned char Unit,
                    uint8_t digits) {
