...
void CheckRotaryEncoder(void) {
  ...
  uint8_t iim;
  uint8_t taste;
  uint8_t open_state;  // flag is set, if many times a open state is found fpr
                       // both switches
  uint8_t close_state; // flag is set, if many times a closed state is found for
                       // both switches
  ...
  R_PORT = (1 << PIN_RL1) | (1 << PIN_RL2) |
           (1 << PIN_RL3); // all ports with 470k to VCC
  ...
  for (kk = 0; kk < 63001; kk++) {
    ...
    if ((ii & (1 << TP2)) != 0)
      index += 1; // index for first switch open is 1 or 3
    if ((ii & (1 << TP3)) != 0)
      index += 2; // index for second switch open is 2 or 3
    if (kk == 0) {
    ...
    if ((RST_PIN_REG & (1 << RST_PIN)) == 0)
      goto end_rotary;
    if ((TP13tab[0] > 20) && (TP13tab[1] > 20) && (TP13tab[2] > 20) &&
        (TP13tab[3] > 20)) {
      switch1 = TP1;
      switch2 = TP3;
      switchm = TP2;
    ...
    if ((TP23tab[0] > 20) && (TP23tab[1] > 20) && (TP23tab[2] > 20) &&
        (TP23tab[3] > 20)) {
      switch1 = TP2;
      switch2 = TP3;
      switchm = TP1;
    ...
    if ((TP12tab[0] > 20) && (TP12tab[1] > 20) && (TP12tab[2] > 20) &&
        (TP12tab[3] > 20)) {
      switch1 = TP1;
      switch2 = TP2;
      switchm = TP3;
  ...
  if (switch1 == switch2)
    return; // no rotary encoder found
  ...
  for (ii = 0; ii < 120; ii++) {
    ...
    if (((iim == 0) && (close_state != 0)) ||
      ...
      lcd_space();
      taste =
          wait_for_key_ms(400); // this state is monitored only for a short time
    }
    if (taste != 0)
      break;
  } /* end for ii */
