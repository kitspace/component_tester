...
// Then a series of up to 1000 load pulses with 1ms duration each is done across
// the R_L (680Ohm)
// resistor.
...
//#ifdef AUTO_CAL
//  pin_combination = ((HighPin - TP_MIN) * 3) + LowPin - TP_MIN - 1;	//
...
//#endif
  ...
  residual_voltage =
      ReadADC(HighPin) - ReadADC(LowPin); // capacitor voltage before any load
...
#define MIN_VOLTAGE 300
  while (ovcnt16 < MAX_LOAD_TIME) {
    ...
    cap_voltage1 =
        ReadADC(HighPin) - residual_voltage; // voltage loaded to capacitor
  ...
  if ((ovcnt16 == 1) && (cap_voltage1 > 1300)) {
    goto keinC; // Voltage of more than 1300mV is reached in one pulse, too fast
                // loaded
  }
  ...
  // time
  //  cap.cval_uncorrected.dw *= GetRLmultip(cap_voltage1);	// get factor to
  //  convert time to capacity from table
  cap.cval_uncorrected.dw *= GetRLmultip(
      cap_voltage1 +
      cap_voltage1 / 40); // get factor to convert time to capacity from table
...
// shown
//   cap.cval -= ((cap.cval * C_H_KORR) / 1000);	// correct with C_H_KORR with
//   0.1% resolution, but prevent overflow
//   cap.cval /= 10;
#ifdef WITH_MENU
  cap.cval *= (1000 - (int8_t)eeprom_read_byte(
                          (uint8_t *)&big_cap_corr)); // correct with actual
                                                      // big_cap_korr at 0.1%
                                                      // resolution
#else
