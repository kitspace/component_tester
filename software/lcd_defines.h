...
#define lcd_write_init(data_length)                                            \
  _lcd_hw_write(0x80, CMD_SetIFOptions | (data_length << 4))
#define lcd_cursor_on()       // ignored
#define lcd_cursor_off()      // ignored
#define CMD_CLEAR 0x01        /* clear display, basic Instruction */
#define CMD_CGRAM_SELECT 0x02 /* SR=0, enable CGRAM address */
#define CMD_SET_ENTRY_MODE                                                     \
  0x04 /* set cursor posistion and shift for read/write */
...
#define GRAPHIC_DISPLAY_ON                                                     \
  0x02 /* Bit 1 is used for graphic display on, only extended Instruction */
#define CMD_SET_GDRAM_ADDRESS                                                  \
  0x80 /* set graphic display RAM address ,extended Instruction */
 ...
 */
#elif (LCD_ST_TYPE ==                                                          \
       8812) /* not (LCD_ST_TYPE == 7565 || 1306 || 7108 || 7920)) */
// support for PCF8812
#undef SCREEN_WIDTH
...
#define ECMD_SET_HV_STAGES                                                     \
  0x08 /* set multiplication of HV-gen  (0 <= m <= 3) */
#define ECMD_SET_TEMP_COEF                                                     \
  0x04                         /* set temperature coefficient   (0 <= T <= 3) */
 ...
 */
#elif (LCD_ST_TYPE ==                                                          \
       8814) /* not ((LCD_ST_TYPE == 7565 || 1306 || 7108 || 7920 || 8812) */
...
#define CMD1_PowerControl                                                      \
  0x50 // Power Control, set Contrast C5:C4 (instruction table 1, DOGM)
#define CMD1_FollowerControl                                                   \
  0x60 // Follower Control, amplified ratio (instruction table 1, DOGM)
...
#define LCDLoadCustomChar(addr)                                                \
  lcd_command(CMD_SetCGRAMAddress | (addr << 3)) // load Custom-character
...
#if ((LCD_ST_TYPE == 7565) || (LCD_ST_TYPE == 1306) ||                         \
     (LCD_ST_TYPE == 7920) || (LCD_ST_TYPE == 7108) ||                         \
     (LCD_ST_TYPE == 8812) || (LCD_ST_TYPE == 8814) ||                         \
     (LCD_ST_TYPE == 9163) || (LCD_ST_TYPE == 7735) || (LCD_ST_TYPE == 9341))
#define GR_OFFSET1                                                             \
  0x20 /* shift 0xa0 to 0x80, direct behind ASCII table, to save place in font \
          table */
