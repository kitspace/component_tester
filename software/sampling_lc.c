...
// Therefore the flag sawzero is set to 0 at start and the detection is
// additionally
...
#ifndef CxL
#define CxL                                                                    \
  18040 // default value for parallel capacitor for inductance measurement
#endif
const uint16_t lc_cpar_ee EEMEM =
    CxL; // place for lc_cpar as calibration constant in eeprom
...
// returns measured period, with 6 bits of fraction, or 256*64 if no resonance
// found
{
  unsigned int xx;
  ...
  unsigned char dist;    // dist should be set automatically to about 1/4 of the
                         // detected period
...
// note: uu[] can be assumed < 600 or so, since we're not interested in peaks
// above some 600 mV
...
// delta is between -20000 and 20000; near zero crossing delta can't exceed one
// uu[] value, i.e. +/- 600
// delta can therefore safely be shifted <<6, but not <<8
// hence the 6 bits of fraction in the peak location
// on my atmega328p, in some cases most measured peak intervals differ by less
// than about 0.05, so 6 bits of fraction is just enough to not lose precision

//        if ((ipk < 2) && (ii > 160)) break;	// no chance to find a period
#ifdef Special_dist1
    } else {
      // this method gives better interpolation of the peak location (fraction
      // of ii)
    ...
    if ((bb < aa) && (sawzero == 1)) {
      ...
      // For a correct size of delta the uu[ii-dist] and uu[ii+dist] are low
      // against 2*uu[ii].
      // By dividing with 2*uu[ii] alone, the interpolation can be used with
      // dist=1.
      uint16_t kdiv;
      //         kdiv = 2 * uu[ii];
      ...
      {
        if (sum_ab < (3 * dist))
          break; // stop if peak not significantly high
#if PeakSearchMethod == 1
        if (ipk == 1)
#else
        if (ii < (dist * 4))
...
#endif
        {
          ...
          if ((ipk > (MinPK + 1)) &&
              (last_per > (mean_per + mean_per / 2 + 32)))
            break; // gap between peaks
          // sanity check: distance between peaks is expected to be 4*dist
          // samples
          unsigned char smp_per;
          smp_per = last_per >> 8; // period with 4 sample units
          if (smp_per >
              dist) { // dist is lower than 1/4 period without rounding
      ...
      if (ipk > MinPK) {
        mean_per = (prevpeak_x - firstpeak_x + ((ipk - MinPK) >> 1)) /
                   (ipk - MinPK); // average period with rounding
      }
#if PeakSearchMethod != 1
    illegal_peak:
#endif
      sawzero = 0;
      if (ipk > Maxpk)
        break; // count of requested peaks is found
    }          /* end found maximum */
    prevdelta = delta;
  } /* end for ii */

  /* total data are analysed, ipk is now the count of peaks + 1 */
  //   if (firstzero>(per>>6)) return 0;     // sanity check: first zero should
  //   be within first period
  if (qptr) {
    // calculate r = ratio of amplitude between two consecutive peaks (*1000 for
    // scaling)
    // but we use all peaks seen in this calculation for better accuracy
    // r is weighted average of peak2/peak1, peak3/peak2 and so on, weighed by
    // respectively peak1, peak2 and so on
    // this can be calculated as r =
    // (peak2+peak3+...+peak_m)/(peak1+peak2+peak_{m-1})
    rr = 0;
    if (ipk > MinPK) {
      ...
      // note scaling: the get_log is *1000, and we now compute Q in multiples
      // of .1, hence the numerator being 10000*pi
    }
...
void sampling_lc(byte LowPin, byte HighPin) {
  uint16_t lc_cpar; // value of parallel capacitor used for calculating
                    // inductance, in pF
  uint16_t period;
  lc_cpar = eeprom_read_word((uint16_t *)&lc_cpar_ee);

  //###################################################################################################
  // new version of the code, with pulses via the ADC port, i.e., without 680
  // ohm series resistor
  ...
  if ((PartFound != PART_RESISTOR) || (inductor_lpre > 0)) {
    ...
    // the other reason is a too big resistance, 2100 Ohm is found by
    // ReadInductance
    return;
  }
  ...
  samplingADC((1 << smplADC_span) | (1 << smplADC_direct), uu, 255, HiPinR_L, 0,
              0, HiPinR_L);

  // also measure some 20 samples at the "cold side" of the coil, and subtract
  // at highest frequencies, this is useful because the on-chip ADC has an RC
  // lowpass which gives an exponentially decaying "DC" offset
  unsigned int uu0[20];
  ADMUX = LowPin | ADref1V1; // switch to "cold" side for reference measurement
  samplingADC((1 << smplADC_span) | (1 << smplADC_direct), uu0, 20, HiPinR_L, 0,
              0, HiPinR_L);
  for (i = 0; i < 20; i++) {
    if (uu[i] >= uu0[i])
      uu[i] -= uu0[i];
  ...
  //   uart_newline(); for (i=0;i<255;i++) { myuart_putc('A'); myuart_putc(' ');
  //   uart_int(uu[i]); uart_int(uu0[i]); uart_newline(); wdt_reset(); }

  byte dist0; // estimate of duration of 1/4 of a period, used to set averaging
              // interval in peaksearch()
  unsigned shift = 0; // by how many bit positions measured period needs to be
                      // shifted due to measuring with span>1

  // check how long until signal reaches 0: that gives us a first guess of 1/4
  // of the resonance period (because we apply an impulse, so we start at the
  // maximum of the sinewave)
  period = peaksearch(uu, NULL);
  dist0 = 1 + (period >> 8); // >>6 because of fraction bits, plus >>2 because
                             // dist0 should be about a quarter period, plus +1
                             // to round up
#if (DEB_SAM == 10)
  lcd_set_cursor(5, 0);
  uart_newline();
  ...
  uint16_t par =
      (1 << smplADC_span) | (1 << smplADC_direct); // default: one pulse
#if (DEB_SAM == 6)
  lcd_set_cursor(5, 0);
  uart_newline();
  lcd_data('d');
...
#define samplingADC_direct (1 << smplADC_direct)
  ...
  if (dist0 > 16) {
    // rather slow resonance: then re-sample with 4 or 16 times larger interval;
    // shift variable serves to take this into account in later calculations
    if (dist0 < 64) {
      shift = 2;
  ...
  // we take the average of 8 measurements, to increase S/N, except when using
  // span>1, since then the sampling takes annoyingly long and S/N usually is
  // better anyway at these lower frequencies
  for (i = 0; i < 8; i++) {
    wdt_reset();
  ...
  if ((par >> smplADC_span) < 4) {
    // case of high frequency: subtract reference measurement, and do no scaling
    // down of accumulated amplitudes since they are small in this case
    ADMUX =
        LowPin | ADref1V1; // switch to "cold" side for reference measurement
    par &= ~samplingADC_cumul;
    for (i = 0; i < 8; i++) {
    ...
    i = 20;
    while (
        i--) // equivalent to for (i=0;i<20;i++) but saves 48 bytes of flash????
    {
      if (uu[i] >= uu0[i])
        uu[i] -= uu0[i];
      else
  ...
  // low frequency: no reference measurement needed, but do scale amplitude by
  // number of accumulated samples
  i = 255;
  while (i--)
    uu[i] >>= 3; // divide all samples by 8
//   }

noavg:;
  period = peaksearch(uu, &lc_qx);
  if (period > (255 << 6))
    period = 0; // set implausible period to zero
#if (DEB_SAM == 5)
  lcd_set_cursor(6, 0);
  uart_newline();
  lcd_data('#');
...
//   = 1/(2*pi*fclock)**2 / C * (d**2)
  ...
  unsigned long vv;
  vv = (unsigned long)period; // measured period with 6 fraction bits, before
                              // applying shift, is < 256*64 = 2^14
  vv = vv * vv;               // vv < 2^28   ; this is (except for shift)  d<<12
#if F_CPU == 20000000UL
  // 1e21 / (2*pi*20e6)**2 / (8 * 1024)      = 7.730192844, which can be
  // computed
  // with a divide by 63 and a mul with 487 (= 7.730158) .
  // with better accuracy 3610/467          (= 7.730192719)
  vv = (vv / 63) * 487;
#elif F_CPU == 16000000UL
  //   vv=(vv>>10)*12368;		// vv < 2^32   ; is (d<<2)/(2*pi*fclock)^2 * 1e21
  //   >>3
  // that 12368 is calculated as 1/(2*pi*16e6)**2*1e21 /8, for 16 MHz CPU clock
  // 1e21 / (2*pi*16e6)**2 / (8 * 1024)      = 12.07842632, which can be
  // computed
  // with a divide by 51 and a mul with 616 (= 12.07843137) .
  // whith better accuracy than   12368/1024 = 12.07812500
  vv = (vv / 51) * 616;
#elif F_CPU == 8000000UL
  //   v=(v>>12)*49473;		// for 8 MHz CPU clock, it's 49473, but we need to
  //   right-shift further to fit in 32 bit
  // 1e21 / (2*pi*16e6)**2 / (8 * 1024)      = 12.07842632, which can be
  // computed
  // with a divide by 51 and a mul with 616 (= 12.07843137) .
...
#error "CPU clocks other than 8 and 16 MHz not yet supported for SamplingADC"
  ...
  // not nice: the nH number will always be even; then again, do we really
  ...
  // probably search of
  if (inductor_lx > 2) {
    // if traditional measurement gave some meaningful-looking value ( > 20 uH,
    // but that's rather arbitrary)
    // discard the new one, it's probably self-resonance
    // note that if a sizeable cap is in parallel, the normal measurement
    // doesn't come up with an answer
    lc_lx = 0;
  }
  ...
  if (inductor_lpre >= 0)
    inductor_lpre = -1; /* no ESR measurement! */
  return;
} /* end of sampling_lc() */
...
#define C_DIFF_LIMIT 200 /* Tolerance of capacity to rewrite eeprom */
  ...
  do {
    ...
    i++;
    //      if ((lc_cpartmp<9500) || (lc_cpartmp>33000)) i=0;		// unstable
    //      or not connected
    if ((lc_cpartmp < 3900) || (lc_cpartmp > 34000))
      i = 0; // unstable or not connected
#ifdef AUTO_LC_CAP
    if ((lc_cpartmp == 0) && (ww > 249))
      break; // a coil is connected in the AUTO_LC_CAP mode
#endif
    if (i > 4) { // Cx measurement was stable
      ...
      if ((lc_cpartmp < (lc_cpar - C_DIFF_LIMIT)) ||
          (lc_cpartmp > (lc_cpar + C_DIFF_LIMIT)))
