...
/* return value: !=0 == key is pressed for x*10ms, 0 == no key pressed, time has
 * expired */
...
#define WaitRotary wait1ms
  ...
  rotary.state[(rotary.ind + ROT_MSK - 2) & ROT_MSK] =
      ' ' - '0'; // for debugging
#endif
#endif
  ...
  while (count_time >= 0) {
    ...
    if (key_pressed == 0x3f) {
      ...
      if ((key_pressed == 0) && (key_cs != 0))
        break; // return, if key is released again
    }
    wdt_reset();
    //     wait_time++;
    if (count_time > 0) { // count only, if counter > 0
      ...
      if (count_time == 0)
        count_time = -1; // never count to zero, zero is endless!
    }
#ifdef WITH_ROTARY_SWITCH
    if (rotary.incre != 0) {
      ...
      if (rotary_switch_present == 0) {
        ...
        //         eeprom_write_byte((uint8_t *)&EE_RotarySwitch,
        //         rotary_switch_present);  // make rotary switch permanent
        //         detected
      }
    }
    if ((wait_time > 20) && (rotary.count != 0))
      break;
    if (wait_time > 20)
      wait_time = 0; // reset time window
#endif
...
void check_rotary(void) {
  ...
  if ((ROTARY_A_REG & (1 << ROTARY_A_PIN)) != 0)
    new_state = 1; // switch A is high
  if ((ROTARY_B_REG & (1 << ROTARY_B_PIN)) != 0)
    new_state |= 2;           // switch B is high
  if (rotary.ind > ROT_MSK) { // index is out of range, initialize
                              //    rotary.state[0] = new_state;
    rotary.state[0] = '>' - '0';
    rotary.state[1] = new_state;
    //    for (rotary.ind=2; rotary.ind<16;rotary.ind++)
    //    rotary.state[rotary.ind] = ' ' - '0';
  ...
  if (rotary.state[rotary.ind] != new_state) {
    ...
    // If the WITH_ROTARY_SWITCH is set to 5, all right state changes cause a
    // count of rotary.count
    // This setting can be used for rotary switch without indexed positions
    // (intends), to get
    // the highest resolution. It can also be used for rotary switches, which
    // have four times
    // more indexed positions than switch cycles per turn (360 degree) for
    // counting  every
    // indexed position.
    // If the WITH_ROTARY_SWITCH is set to 1, only 2 of 4 state changes are
    // counted.
    // If the WITH_ROTARY_SWITCH is set to 2 or 3, only one of 4 state changes
    // are counted.
    // Use setting of 2 or 3, if you operate a rotary switch type, which has the
    // same count of
    // indexed positions as switch cycles per turn to get one count for every
    // indexed position.

    // There are three states to monitor, the new_state, the last
    // rotary.state[rotary.ind] and
    // the state before rotary.state[old_ind] .
    old_ind = (rotary.ind + ROT_MSK) &
              ROT_MSK; // the index of the state before rotary.ind
#if WITH_ROTARY_SWITCH != 3
    // type 1, 2 and 5
    if ((new_state == 0) && (rotary.state[rotary.ind] == 2) &&
        (rotary.state[old_ind] == 3)) {
      // 320 +
      rotary.count++;
    }
    if ((new_state == 0) && (rotary.state[rotary.ind] == 1) &&
        (rotary.state[old_ind] == 3)) {
      // 310 -
      rotary.count--;
    }
    ...
    // type 1, 3 and 5
    if ((new_state == 3) && (rotary.state[rotary.ind] == 2) &&
        (rotary.state[old_ind] == 0)) {
      // 023 -
      rotary.count--;
    }
    if ((new_state == 3) && (rotary.state[rotary.ind] == 1) &&
        (rotary.state[old_ind] == 0)) {
      // 013 +
      rotary.count++;
    }
    ...
    if ((new_state == 1) && (rotary.state[rotary.ind] == 3) &&
        (rotary.state[old_ind] == 2)) {
      // 231 -
      rotary.count--;
    }
    if ((new_state == 2) && (rotary.state[rotary.ind] == 3) &&
        (rotary.state[old_ind] == 1)) {
      // 132 +
      rotary.count++;
    }
    if ((new_state == 1) && (rotary.state[rotary.ind] == 0) &&
        (rotary.state[old_ind] == 2)) {
      // 201 +
      rotary.count++;
    }
    if ((new_state == 2) && (rotary.state[rotary.ind] == 0) &&
        (rotary.state[old_ind] == 1)) {
      // 102 -
      rotary.count--;
    }
  ...
  // special handling with separate up and down switches instead of rotary
  // switch
  if (rotary.ind > ROT_MSK) { // index is out of range, initialize
    rotary.a_state = 0x55;
    rotary.b_state = 0x55;
  ...
  if (last_line_used != 0) {
    ...
    do {
      if (wait_for_key_ms(SHORT_WAIT_TIME) > 0)
        break;
      // continue waiting, if the key is not pressed, but rotary switch is
      // rotated
    } while (rotary.incre > 0);
#else
    wait_for_key_ms(
        SHORT_WAIT_TIME); // wait until time is elapsed or key is pressed
#endif
