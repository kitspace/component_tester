...
//=================================================================
// measure frequency at external pin T0 (PD4@mega328 or PB0@mega644) with
// Counter 0
...
#define SCALER_POSITION (LCD_LINE_LENGTH - 5)
  ...
  for (mm = 0; mm < 240; mm++) {
    ...
    // *************************************************************************
    // set up Counter 0
    // Counter 0 is used to count the external signal connected to T0 (PD4 or
    // PB0)
...
#warning F_CPU can not be divided by 256, measured frequency is wrong!
...
#endif
    OCR1B = (CNT1_END_VAL &
             0xffff);        // set to 1 second  (counter 0 is started with 1)
    ...
    TIMSK1 =
        (1 << OCIE1B) |
        (1
         << OCIE1A); // enable the Compare A match and Compare B match interrupt
#endif
    ...
    for (ii = 0; ii < 50; ii++) {
      ...
      if ((TIFR1 & (1 << TOV1)) != 0) {
        TIFR1 =
            (1 << OCF1B) |
            (1 << TOV1); // reset TOV1 Overflow and Output compare B match Flag
        TIMSK1 = (1 << OCIE1B); // enable the Compare B match interrupt
      }
#endif
      wdt_reset();
      if (!(RST_PIN_REG & (1 << RST_PIN)))
        taste = 1; // user request stop of operation
#if PROCESSOR_TYP == 1280
      if (TCCR3B == 0)
        break; // timer 3 is stopped by interrupt
#else
      if (TCCR0B == 0)
        break;  // timer 0 is stopped by interrupt
#endif
    }
...
// ATmega328 ...
    ...
    if (TCCR1B != 0) {
      ...
      // When stopped with the for loop, the time is too long because wait call
      // does not
    ...
    if ((ext_freq.dw < FMAX_PERIOD) && (ext_freq.dw > 0)) {
      ...
      // ********************
      // *************************************************************************
      pinchange_max = ((10 * (unsigned long)ext_freq.dw) + MHZ_CPU) /
                      MHZ_CPU;            // about 10000000 clock tics
      ...
      sei();
      PCMSK_FREQ |=
          (1 << PCINT_FREQ); // monitor PD4 PCINT20 or PB0 PCINT8 pin change
      for (ii = 0; ii < 250; ii++) {
        wait20ms();
        wdt_reset();
        if (!(RST_PIN_REG & (1 << RST_PIN)))
          taste = 1; // user request stop of operation
        if ((PCMSK_FREQ & (1 << PCINT_FREQ)) == 0)
          break; // monitoring is disabled by interrupt
      }          /* end for ii */
#if PROCESSOR_TYP == 1280
      ...
      PCMSK_FREQ &= ~(
          1 << PCINT_FREQ); // stop monitor PD4 PCINT20 or PB0 PCINT8 pin change
...
//	---------------------------------------
      ...
      lcd_data('T');
      lcd_equal(); // lcd_data('=');
      ext_period = ((unsigned long long)ext_freq.dw * (200000 / MHZ_CPU)) /
                   pinchange_max;
#if PROCESSOR_TYP == 644
      if ((FDIV_PORT & (1 << FDIV_PIN)) != 0) {
      ...
      if (pinchange_max > 127) {
        DisplayValue(ext_period, -11, 's',
                     7); // show period converted to 0.01ns units
      } else {
        // prevent overflow of 32-Bit
        DisplayValue((unsigned long)(ext_period / 100), -9, 's',
                     7); // show period converted to 1ns units
      }
      //	---------------------------------------
      if (ii == 250) {
...
//	      ----------------------------------------------------
#if (LCD_LINES > 3)
          lcd_line4(); // use line 4 of 4-line LCD to report the computed
                       // frequency
#else
          lcd_line1(); // overwrite line 1 of 2-line LCD to report the computed
                       // frequency
#endif
          lcd_data('f');
          lcd_equal(); // lcd_data('=');
          if (ext_period > 1000000000) {
            ...
            freq_from_per =
                (unsigned long long)(100000000000000000) / ext_period;
            DisplayValue(freq_from_per, -6, 'H',
                         7); // display with  0.000001 Hz resolution
          } else {
            ...
            DisplayValue(freq_from_per, -3, 'H',
                         7); // display with  0.001 Hz resolution
          }
          lcd_data('z');
          FREQINP_DDR &= ~(1 << FREQINP_PIN); // switch frequency pin to input
          //	      ----------------------------------------------------
    ...
    if ((FDIV_PORT & (1 << FDIV_PIN)) == 0) {
      ...
      if (((freq_count >= FMAX_PERIOD) &&
           (freq_count < ((unsigned long)FMAX_PERIOD * FREQ_DIV))) ||
          (freq_count > FMAX_INPUT)) {
        FDIV_PORT |=
            (1 << FDIV_PIN); // use frequency divider for next measurement
      }
    } else {
      // frequency divider is activ
...
/* ************************************************************ */
// this handler is also used by SamplingADC.S, where simply any dummy interrupt
// handler is usable; but if timing of this handler changes, need to adapt
// SamplingADC.S as well
ISR(TIMER1_COMPA_vect, ISR_BLOCK) {
#if PROCESSOR_TYP == 1280
  TCCR3B = (1 << CS32) | (1 << CS31) |
           (0 << CS30); // start the counter 3 with external input T3
#else
  TCCR0B = (1 << CS02) | (1 << CS01) |
           (0 << CS00);   // now start the counter 0 with external input T0
#endif
}
...
ISR(PCINTx_vect, ISR_BLOCK) {
  ...
  if (pinchange_count >= pinchange_max) {
    ...
    TCCR0B = 0;                       // stop counter 0
    PCMSK_FREQ &= ~(1 << PCINT_FREQ); // disable monitoring of PD4 PCINT20 or
                                      // PB0 PCINT8 pin change
#endif
    //     PCICR &= ~(1<<PCI_ENABLE_BIT);	// disable the interrupt
  }
  pinchange_count++;
