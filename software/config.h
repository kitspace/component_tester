...
*/
     ...
     (LCD_ST_TYPE == 7108) || (LCD_ST_TYPE == 8812) ||                         \
     (LCD_ST_TYPE == 8814) || (LCD_ST_TYPE == 7735) ||                         \
     (LCD_ST_TYPE == 9163) || (LCD_ST_TYPE == 9341))
...
#include "part_defs.h"
//#define DebugOut 3		// if set, output of voltages of resistor
//measurements in row 2,3,4
//#define DebugOut 4		// if set, output of voltages of Diode
//measurement in row 3+4
//#define DebugOut 5		// if set, output of Transistor checks in row
//2+3
//#define DebugOut 10		// if set, output of capacity measurements
//(ReadCapacity) in row 3+4
//#define DebugOut 11		// if set, output of load time (ReadCapacity)
//in row 3 for C at pins 1+3
...
#define MODE_I2C 2      /* I2C interface for SSD1306 */
                        /* SCL + SDA */
#define MODE_3LINE                                                             \
  3 /* special 3 line serial output with Data/Command as first data bit        \
       PCF8814 */
/* RES + EN + B0 (+ CE) */
...
#define MODE_7920_SERIAL 5 /* serial interface for ST7920 */
                           /* EN + B0 (+ RESET) */
#define MODE_7108_SERIAL                                                       \
  6 /* serial interface for ST7108 with shift register 74HC164 */
...
/*
  ...
  The TPext can be used with a 10:1 resistor divider as external voltage probe
  up to 50V
*/
/* ************************************************************************* */
...
#if PROCESSOR_TYP == 644
/* ------------------------------------------------------------------------- */
/* define the input pin for frequency measuring and also the pin change
 * monitoring port for measuring the periode */
...
// can be preset to PB6 for character display and PB3 for ST7565 graphic
// controller
#endif
...
#define FDIV_PIN PC0
/* the two bits P0 and P1 at FINP port control the input of frequency
 * measurement */
/* P1:P0 = 00 external input, P1:P0 = 10 high frequency crystal, P1:P0 = 11 low
 * frequency crystal */
...
// can be preset to PD2 for character display and PD5 for ST7565 graphic
// controller
#endif
#endif /* PROCESSOR_TYP */
...
// option STRIP_GRID is used to specify other connection of the LCD (chinese
// version),
...
// alternative connection of graphical LCD for the chinese T4 board with PD0
// <--> LCD-REST patch
...
#ifndef LCD_I2C_ADDR
#define LCD_I2C_ADDR                                                           \
  0x3C /* SSD1306 controller defines 0x3c or 0x3d (SA0=1) as address */
#endif
/* --------------------------------------------- */
 ...
 */
    ...
    (LCD_INTERFACE_MODE == MODE_1803_SERIAL)
...
/* RS_PIN is connected to LCD-RS and serial data input of the '164 shift
 * register */
 ...
 */
/* CS2_PIN is connected to the LCD-CS2  (chip select of controller 2, row
 * 64-127) */
...
/* --------------------------------------------- */
#if PROCESSOR_TYP ==                                                           \
    644 /* connection for 4-bit parallel interface and mega324/644/1284 */
#if defined(STRIP_GRID_BOARD) && ((STRIP_GRID_BOARD & 0xef) != 0)
// special Layout for strip grid board with the 4-bit parallel interface and
// ATmega324/644/1284
#define HW_LCD_EN_PORT PORTB
#define HW_LCD_EN_PIN 2
...
/* --------------------------------------------- */
#elif PROCESSOR_TYP ==                                                         \
    1280 /* normal layout with 4-bit parallel interface and mega1280/2560 */
#ifdef STRIP_GRID_BOARD
// special connection for strip grid board with 4-bit parallel interface and
// ATmega1280
#define HW_LCD_EN_PORT PORTA
#define HW_LCD_EN_PIN 5
...
// With the option NO_CAP_HOLD_TIME you specify, that capacitor loaded with 680
// Ohm resistor will not
// be tested to hold the voltage same time as load time.
// Otherwise (without this option) the voltage drop during load time is
// compensated to avoid displaying
// too much capacity for capacitors with internal parallel resistance.
// #define NO_CAP_HOLD_TIME

// U_SCALE can be set to 4 for better resolution of ReadADC function for
// resistor measurement
#define U_SCALE 4

// R_ANZ_MESS can be set to a higher number of measurements (up to 200) for
// resistor measurement
...
// TP2_CAP_OFFSET is a additionally offset for TP2 capacity measurements in pF
// units
#define TP2_CAP_OFFSET 2

// CABLE_CAP defines the capacity (pF) of 12cm cable with clip at the terminal
// pins
#define CABLE_CAP 3
...
// MIDDLE_WAIT_TIME is used for display the next parameter for Show data menu
// function
...
/*
With define SWUART_INVERT you can specify, if the software-UART operates normal
or invers.
in the normal mode the UART sends with usual logik level (Low = 0; High = 1).
You can use this mode for direct connection to a µC, or a level converter like
MAX232.

With invers mode the UART sends with invers logik (Low = 1, High = 0).
This is the level of a standard RS232 port of a PC.
In most cases the output of the software UART can so be connected to the RxD of
a PC.
The specification say, that level -3V to 3V is unspecified, but in most cases it
works.
...
#endif
#else /* no WITH_UART */
// If you use any pin of port C for output, you should define all used pins in
// TXD_MSK.
// With TXD_VAL you can specify the default level of output pins for all port C
// output pins.
#define TXD_MSK 0
#define TXD_VAL 0
#endif /* WITH_UART */
...
// all capacity measurement results does not go C_LIMIT_TO_UNCALIBRATED pF below
// the calibrated zero capacity.
#define C_LIMIT_TO_UNCALIBRATED 20

// all ESR measurements results does not go R_LIMIT_TO_UNCALIBRATED 0.01 Ohm
// units below the calibrated zero resistance.
...
#define smplADC_freq                                                           \
  7 /* generate a bunch of pulses (only without -DSamplingADC_CNT ) */
...
#define smplADC_mux                                                            \
  3 /* with large span, switch MUX to input 0 for ADC measurements that are to \
       be discarded */
#define smplADC_sck                                                            \
  1 /* short-circuit the crystal after excitation, duration given by extra     \
       parameter */
#define smplADC_8bit 0 /* treat the array as an array of 8-bit samples */
#define smplADC_hpf                                                            \
  2 /* treat the data with a high-pass filter, i.e., remove DC; only to be     \
       used with _8bit, in which case the 8bit samples are signed */
