...
/* for resistor and other measurements 190 ADC reads are done with more than
 * 21ms  */
...
get_sample:
  ...
  if (Probe & (1 << REFS1))
    Probe &=
        ~(1 << REFS0); /* ATmega640/1280/2560 1.1V Reference with REFS0=0 */
#endif
  ...
  while (samples < ADCconfig.Samples) /* take samples */ {
    ...
    if ((samples == 4) && (ADCconfig.U_Bandgap > 255) &&
        ((uint16_t)Value < 1024) && !(Probe & (1 << REFS1))) {
  ...
  return U;
  //   return ((unsigned int)(Value / (1023 * (unsigned
  //   long)ADCconfig.Samples)));
}
