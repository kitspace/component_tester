...
#define MAX_REP 4
  ...
  if ((test_mode & 0xf0) == 0) {
    ...
    if (wait_for_key_ms(2000) > 10)
      goto begin_selftest; // key is pressed again
#ifdef WITH_MENU
  } else {
    // report to user, that probes should be shorted
    ww = 0;
    for (tt = 0; tt < 150;
         tt++) { /* wait about 30 seconds for shorted probes */
      lcd_clear();
...
begin_selftest:
  ...
  eeprom_write_byte((uint8_t *)(&EE_ESR_ZEROtab[2]),
                    (uint8_t)0); // clear zero offset
  eeprom_write_byte((uint8_t *)(&EE_ESR_ZEROtab[3]),
                    (uint8_t)0); // clear zero offset
  eeprom_write_byte((uint8_t *)(&EE_ESR_ZEROtab[1]),
                    (uint8_t)0); // clear zero offset
  ...
  eeprom_write_byte((uint8_t *)(&EE_ESR_ZEROtab[2]),
                    (uint8_t)adcmv[0]); // fix zero offset
  if (adcmv[1] >= 90) {
    adcmv[1] = ESR_ZERO; // set back to default value
  }
  eeprom_write_byte((uint8_t *)(&EE_ESR_ZEROtab[3]),
                    (uint8_t)adcmv[1]); // fix zero offset
  if (adcmv[2] >= 90) {
    adcmv[2] = ESR_ZERO; // set back to default value
  }
  eeprom_write_byte((uint8_t *)(&EE_ESR_ZEROtab[1]),
                    (uint8_t)adcmv[2]); // fix zero offset
  last_line_used = 2;
...
#define TEST_COUNT 8
  if ((test_mode & 0x0f) == 1) { /* full test requested */
    ...
    for (tt = 1; tt < TEST_COUNT; tt++) { // loop for all Tests
      for (ww = 0; ww < MAX_REP; ww++) {  // repeat the test MAX_REP times
        ...
        lcd_space();
        //############################################
        if (tt == 1) { // output of reference voltage and factors for capacity
                       // measurement
          ...
          ADCconfig.Samples =
              R_ANZ_MESS; // set number of ADC reads near to maximum
        }
        //############################################
        if (tt == 2) { // how equal are the RL resistors?
          u680 = ((long)ADCconfig.U_AVCC * (PIN_RM + R_L_VAL) /
                  (PIN_RM + R_L_VAL + R_L_VAL + PIN_RP));
        ...
        //############################################
        if (tt ==
            6) { // can we switch the ADC pins to VCC across the R_H resistor?
        ...
        if (tt == 7) { // is the voltage of all R_H / R_L dividers correct?
          u680 = ((long)ADCconfig.U_AVCC * (PIN_RM + R_L_VAL) /
                  (PIN_RM + R_L_VAL + (unsigned long)R_H_VAL * 100));
          R_PORT = 1 << PIN_RH1;                   // RH1 to VCC
        ...
        if (tt > 1) {      // output 3 voltages
          lcd_line2();     // Cursor to column 1, row 2
          i2lcd(adcmv[0]); // lcd_string(itoa(adcmv[0], outval, 10));	//output
                           // voltage 1
          lcd_space();
          i2lcd(adcmv[1]); // lcd_string(itoa(adcmv[1], outval, 10));	//output
                           // voltage 2
          lcd_space();
          i2lcd(adcmv[2]); // lcd_string(itoa(adcmv[2], outval, 10));	//output
                           // voltage 3
        }
    ...
    ResistorVal[0] =
        (adcmv[0] * (unsigned long)R_L_VAL) / (adcmv[1] - adcmv[0]);
    DisplayValue(ResistorVal[0], -1, LCD_CHAR_OMEGA, 3);
    lcd_space();
    lcd_data('H');
    lcd_equal(); // lcd_data('=');
    ResistorVal[1] =
        (vcc_diff(adcmv[1]) * (unsigned long)R_L_VAL) / (adcmv[1] - adcmv[0]);
    ...
    ADCSRB = 0; // switch back to lower 8 MUX inputs
    ResistorVal[1] =
        (vcc_diff(adcmv[0]) * (unsigned long)R_L_VAL) / (adcmv[0] - adcmv[1]);
    DisplayValue(ResistorVal[1], -1, LCD_CHAR_OMEGA, 3);
    lcd_space();
    lcd_data('L');
    lcd_equal(); // lcd_data('=');
    ResistorVal[0] =
        (adcmv[1] * (unsigned long)R_L_VAL) / (adcmv[0] - adcmv[1]);
    DisplayValue(ResistorVal[0], -1, LCD_CHAR_OMEGA, 3);
  ...
  adcmv[5] =
      (unsigned int)
          cap.cval_uncorrected.dw; // save capacity value of empty Pin 1:3
  ReadCapacity(TP3, TP2);
  adcmv[6] =
      (unsigned int)
          cap.cval_uncorrected.dw; // save capacity value of empty Pin 2:3
  ReadCapacity(TP2, TP1);
  adcmv[2] =
      (unsigned int)
          cap.cval_uncorrected.dw; // save capacity value of empty Pin 1:2
  ReadCapacity(TP1, TP3);
  adcmv[1] =
      (unsigned int)
          cap.cval_uncorrected.dw; // save capacity value of empty Pin 3:1
  ReadCapacity(TP2, TP3);
  adcmv[4] =
      (unsigned int)
          cap.cval_uncorrected.dw; // save capacity value of empty Pin 3:2
  ReadCapacity(TP1, TP2);
  adcmv[0] =
      (unsigned int)
          cap.cval_uncorrected.dw; // save capacity value of empty Pin 2:1
#ifdef WITH_MENU
  ...
  {
    adcmv[3] =
        adcmv[0] + 2; // mark as uncalibrated until Cap > 100nF has success
  } else {
  ...
  u2lcd_space(
      adcmv[5]); // DisplayValue(adcmv[5],0,' ',3);		//output cap0
                 // 1:3
  u2lcd_space(
      adcmv[6]); // DisplayValue(adcmv[6],0,' ',3);		//output cap0
                 // 2:3
  DisplayValue(adcmv[2], -12, 'F', 3); // output cap0 1:2
#ifdef AUTO_CAL
  for (ww = 0; ww < 7; ww++) { // checking loop
    if ((adcmv[ww] > 190) || (adcmv[ww] < 10))
      goto no_c0save;
  }
  for (ww = 0; ww < 7; ww++) {
    ...
    (void)eeprom_write_byte((uint8_t *)(&c_zero_tab[ww]),
                            adcmv[ww] +
                                (COMP_SLEW1 / (CC0 + CABLE_CAP + COMP_SLEW2)));
  }
  lcd_line2();
...
no_c0save:
  ...
  // calibration (UnCalibrated = 2) or for the full selftest call (test_mode =
  // 1)
  // of the menu function, not with the automatically call (test_mode = 1).
  {
    ...
    for (ww = 0; ww < 64; ww++) {
      ...
      if (cap_found > 4) {
        ...
        i2lcd(load_diff); // lcd_string(itoa(load_diff, outval, 10));	//output
                          // REF_C_KORR
...
#define CAP_ADC TP3 /* Cap >100nF at TP3 */
        ...
        ADCconfig.U_Bandgap = adc_internal_reference;
        udiff = (int8_t)(((signed long)(adcmv[0] + adcmv[2] - adcmv[1])) *
                         adc_internal_reference / adcmv[1]) +
                REF_R_KORR;
        lcd_line2();
        ...
        UnCalibrated = 0;        // clear the UnCalibrated Flag
        lcd_cursor_off();        // switch cursor off
        cap_found = eeprom_read_byte(
            (uint8_t *)&c_zero_tab[0]); // read first capacity zero offset
        eeprom_write_byte((uint8_t *)&c_zero_tab[3],
                          cap_found); // mark as calibrated permanent
...
uint8_t ShortedProbes(uint8_t Probe1, uint8_t Probe2) {
  ...
  R_DDR = pp | pgm_read_byte(addr); // pgm_read_byte(PinRHtab[Probe1]) |
                                    // pgm_read_byte(PinRLtab[Probe2]);
...
#define MAX_UH_DIFF 30
  ...
  URH -= ((long)U_VCC * (long)(PIN_RP - PIN_RM)) /
         (4 * (unsigned long)(R_L_VAL + PIN_RM)); // differenz of Pin resistance
                                                  // high (22) and low (20)
