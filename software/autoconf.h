...
/* defines for PCF8812 */
/* The space at the display is too small for the big icons for this type, some
...
#define LCD_ST7565_H_OFFSET                                                    \
  0 /* default horizontal offset of the display window */
#endif
...
#define LCD_LINE_LENGTH 16
     ...
     (((SCREEN_WIDTH - (TEXT_RIGHT_TO_ICON * FONT_WIDTH)) / FONT_WIDTH) < 9))
...
#ifndef R_L_VAL
#define R_L_VAL                                                                \
  6800 // standard value 680 Ohm, multiplied by 10 for 0.1 Ohm resolution
#endif
#ifndef R_H_VAL
#define R_H_VAL                                                                \
  47000 // standard value 470000 Ohm, multiplied by 10, divided by 100
#endif
...
/* check the U_SCALE setting, U_SCALE is temporary used to increase resolution
 * of ReadADC */
#if U_SCALE < 1
// limit U_SCALE
#undef U_SCALE
...
// numbers of RESIS1 and RESIS2 are swapped for OLED display, which shows a
// corrupt RESIS1 character otherwise ???
#define LCD_CHAR_RESIS1                                                        \
  7 // Resistor left part will be generated as custom character
#define LCD_CHAR_RESIS2                                                        \
  6 // Resistor right part will be generated as custom character
...
// redefine the special symbols for software character set used with graphical
// display
...
// SEARCH_PARASITIC let the Tester search for greater Base-Emitter capacity, if
// two transistors are detected.
// The one with the lower capacity value is the parasitic, the other is shown by
// default
// This is enabled for all processors (deselecting save about 120 bytes flash)
// Otherwise the shown type (NPNp or PNPn) depends on the selected pin sequence
// of the tester!
#if FLASHEND > 0x1fff
...
#if FLASHEND > 0x1fff
// You can save about 328 bytes of Flash, if you don't show the ICE0 and ICEs
// Collector cutoff current.
// Only enabled for mega328, but you can also enable it for mega168, if you
// deselect other functions.
...
/* the hFE (B) can also be determined with  common emitter circuit (not for
 * atmega8 ) */
#if FLASHEND > 0x1fff
#ifndef NO_COMMON_EMITTER_HFE
// only define the common emitter, if the extended tests are not explicitly
// requested
...
#endif
#else
// no COMMON_COLLECTOR hFE measurement methode, the extended tests are always
// possible!
#define EXTENDED_TESTS
#endif
#else
// no COMMON_EMITTER  hFE measurement methode, the extended tests are always
// possible!
#define EXTENDED_TESTS
#endif
...
// Only by selecting one hFE measurement methode, the extended tests are always
// possible!
#define EXTENDED_TESTS
#endif
#else
...
//#define EXTENDED_TESTS  /* extended tests are possible for ATmega168, if the
//gcc option -mcall-prologues is set */
...
#warning                                                                       \
    "Search of parasitic transistor not possible. NPNp or PNPn result depends on the selected pin sequence!"
#endif /* BOTH_HFE */
#ifdef SHOW_ICE
#ifdef BOTH_HFE
...
#if ((F_CPU / 100) / 32) < 256
#define CNTR2_PRESCALER                                                        \
  (0 << CS22) | (1 << CS21) | (1 << CS20) /* prescaler 32 */
...
#define CNTR2_PRESCALER                                                        \
  (1 << CS22) | (0 << CS21) | (1 << CS20) /* prescaler 128 */
...
.macro AOUT adr,
    reg.if _SFR_IO_REG_P(\adr) out _SFR_IO_ADDR(\adr), \reg.else sts     \adr, \reg.endif
                                                                                   .endm
                                                                               ...
                                                                               reg
    ,
    adr.if _SFR_IO_REG_P(\adr) in     \reg,
    _SFR_IO_ADDR(\adr).else lds     \reg, \adr.endif
                                              .endm

                                              /* LDIZ load the address adr to
                                                 the Z register R30:R31 */
                                              .macro LDIZ adr ldi ZL,
    lo8(\adr) ldi ZH, hi8(\adr).endm
...
// #if ((ROTARY_A_DDR == HW_LCD_RES_DDR) && (ROTARY_A_PIN == HW_LCD_RES_PIN)) ||
// ((ROTARY_A_DDR == HW_LCD_EN_DDR) && (ROTARY_A_PIN == HW_LCD_EN_PIN)) ||
// ((ROTARY_A_DDR == HW_LCD_RS_DDR) && (ROTARY_A_PIN == HW_LCD_RS_PIN)) ||
// ((ROTARY_A_DDR == HW_LCD_B0_DDR) && (ROTARY_A_PIN == HW_LCD_B0_PIN)) ||
// ((ROTARY_A_DDR == HW_LCD_CE_DDR) && (ROTARY_A_PIN == HW_LCD_CE_PIN))
//  #warning Option WITH_ROTARY_SWITCH can not be used together with option
//  LCD_SPI_OPEN_COL, if the ROTARY_A switch signals use the same port signals
//  as the LCD.
// #endif
// #if ((ROTARY_B_DDR == HW_LCD_RES_DDR) && (ROTARY_B_PIN == HW_LCD_RES_PIN)) ||
// ((ROTARY_B_DDR == HW_LCD_EN_DDR) && (ROTARY_B_PIN == HW_LCD_EN_PIN)) ||
// ((ROTARY_B_DDR == HW_LCD_RS_DDR) && (ROTARY_B_PIN == HW_LCD_RS_PIN)) ||
// ((ROTARY_B_DDR == HW_LCD_B0_DDR) && (ROTARY_B_PIN == HW_LCD_B0_PIN)) ||
// ((ROTARY_B_DDR == HW_LCD_CE_DDR) && (ROTARY_B_PIN == HW_LCD_CE_PIN))
//  #warning Option WITH_ROTARY_SWITCH can not be used together with option
//  LCD_SPI_OPEN_COL, if the ROTARY_B switch signals use the same port signals
//  as the LCD.
// #endif
#pragma message                                                                \
    "Option WITH_ROTARY_SWITCH can not be used together with option LCD_SPI_OPEN_COL, if the ROTARY switch signals use the same port signals as the LCD."
#endif
