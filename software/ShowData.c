...
show_page_1:
  ...
  if (rotary.incre > FAST_ROTATION)
    return; // fast rotation ends the function
  if (rotary.count < 0)
    goto show_page_1;
show_page_2:
#endif
...
show_page_3:
  ...
  u2lcd_space(eeprom_read_byte(&c_zero_tab[5]) -
              (COMP_SLEW1 / (CC0 + CABLE_CAP + COMP_SLEW2))); // output cap0 1:3
  u2lcd_space(eeprom_read_byte(&c_zero_tab[6]) -
              (COMP_SLEW1 / (CC0 + CABLE_CAP + COMP_SLEW2))); // output cap0 2:3
  DisplayValue16(eeprom_read_byte(&c_zero_tab[2]) -
                     (COMP_SLEW1 / (CC0 + CABLE_CAP + COMP_SLEW2)),
                 -12, 'F', 3); // output cap0 1:2
  lcd_line2();
  lcd_spaces(3);
  u2lcd_space(eeprom_read_byte(&c_zero_tab[1]) -
              (COMP_SLEW1 / (CC0 + CABLE_CAP + COMP_SLEW2))); // output cap0 3:1
  u2lcd_space(eeprom_read_byte(&c_zero_tab[4]) -
              (COMP_SLEW1 / (CC0 + CABLE_CAP + COMP_SLEW2))); // output cap0 3:2
  DisplayValue16(eeprom_read_byte(&c_zero_tab[0]) -
                     (COMP_SLEW1 / (CC0 + CABLE_CAP + COMP_SLEW2)),
                 -12, 'F', 3); // output cap0 2:1
#if (LCD_LINES > 3)
  lcd_line3();
#else
  wait_for_key_ms(MIDDLE_WAIT_TIME);
...
show_page_5:
  ...
  lcd_set_cursor((LCD_LINES - 1) * PAGES_PER_LINE,
                 0); // set for initial clear screen
  for (ii = 0; ii <= 2; ii++)
    for (jj = 0; jj <= 2; jj++)
      if (ii != jj) {
        kk = (ii * 3) + jj - 1;
...
#if (LCD_LINES > 3)
          if (rotary.count < -2)
            goto show_page_1; // page_2 is shown together with _page_1
          if (rotary.count < -1)
            goto show_page_3; // page_4 is shown together with _page 3
          if (rotary.count < 0)
            goto show_page_5;
#else
          if (rotary.count < -4)
