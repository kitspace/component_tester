...
// computes RHmultip, adc_internal_reference (ADCconfig.U_Bandgap) and
// ref_mv_offs
...
#define Ref_Tab_Beginn 1000 // begin of table is 1000mV
  ...
  // RHmultip is the interpolated factor to compute capacity from load time with
  // 470k
  RHmultip =
      ((y1 - y2) * tabres + (Ref_Tab_Abstand / 2)) / Ref_Tab_Abstand + y2;
//########################################
#else
  RHmultip = DEFAULT_RH_FAKT; // default Multiplier for capacity measurement
                              // with R_H (470KOhm)
#endif
...
#ifdef AUTO_CAL
  ADCconfig.U_Bandgap =
      (referenz + (int8_t)eeprom_read_byte((uint8_t *)&RefDiff));
#else
