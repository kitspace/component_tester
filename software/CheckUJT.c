...
ujt_exch_b:;
  ...
  ADMUX =
      Epin |
      (1
       << REFS0); // switch Multiplexer to emitter and use 5V reference voltage
  ...
  //              myuart_putc('s'); myuart_putc(' '); uart_int(port1);
  //              uart_int(ddr1); uart_int(port0); uart_int(ddr0);
  //              uart_newline();
  uint8_t i;
  uint8_t max = 0;
  uint8_t maxm;
  ...
  // found first maximum
  // now check whether after this, the voltage drops significantly, if so, we've
  ...
  for (; i < 64; i++) {
    ...
    if (v < maxm) {
      ...
      v = (r << 8) / (r + R_L_VAL); // Vbb in units of supplyvoltage/256 ;
                                    // perhaps should also take into account the
                                    // about 20 ohm internal resistance?
      max -= 35; // subtract about 0.7 volts (1 diode voltage drop) from the
                 // maximum emitter voltage seen; 35 is 0.7/5*256.
      ntrans.gthvoltage =
          100ul * max /
          v; // this is eta in percent, cf.
...
//   uart_newline(); for (i=0;i<64;i++) { myuart_putc('a'); myuart_putc(' ');
//   uart_int(uu[i<<2]); uart_newline(); wdt_reset(); }
//   uart_newline(); for (i=0;i<64;i++) { myuart_putc('a'); myuart_putc(' ');
//   uart_int(uu[i]); uart_newline(); wdt_reset(); }
//              myuart_putc('v'); myuart_putc(' '); uart_int(min);
//              uart_int(max); uart_newline();
#else /* ! SamplingADC */
  uint16_t v0, v1, v2;
  ...
  R_PORT = pinmaskRL(ntrans.b) |
           pinmaskRL(diodes.Cathode[1]); // UJT emitter and B2 with RL to VCC
  R_DDR = R_PORT;
  ...
  R_PORT = pinmaskRL(ntrans.b) |
           pinmaskRL(diodes.Cathode[0]); // UJT emitter and B2 with RL to VCC
  R_DDR = R_PORT;
