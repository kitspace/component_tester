...
int main(void) {
  ...
  unsigned long n_cval; // capacitor value of NPN B-E diode, for deselecting the
...
//  PRR1 =  (1<<PRTIM3) ;
  ...
  PRR0 = (1 << PRTWI) | (1 << PRSPI) | (1 << PRUSART1);
  PRR1 = (1 << PRTIM5) | (1 << PRTIM4) | (1 << PRTIM3) | (1 << PRUSART3) |
         (1 << PRUSART2) | (1 << PRUSART3);
#else
  PRR = (1 << PRTWI) | (1 << PRSPI) | (1 << PRUSART0);
#endif
  //	disable digital inputs of Analog pins, but TP1-3 digital inputs must be
  //left enabled for VGS measurement
...
// tester display time selection
  ...
  rotary.ind =
      ROT_MSK + 1; // initilize state history with next call of check_rotary()
#endif
...
#endif
  display_time =
      OFF_WAIT_TIME; // LONG_WAIT_TIME for single mode, else SHORT_WAIT_TIME
  if (ii > 30) {
...
/* 2.5V precision reference is checked OK */
  ...
  while (!(RST_PIN_REG & (1 << RST_PIN))) {
    ...
    Display_mV(Vext * EXT_NUMERATOR / EXT_DENOMINATOR,
               3); // Display 3 Digits of this mV units
#else
    DisplayValue((unsigned long)Vext * EXT_NUMERATOR / EXT_DENOMINATOR, -3, 'V',
                 3); // Display 3 Digits of this mV units
#endif
    lcd_refresh();     // write the pixels to display, ST7920 only
  ...
  lcd_cursor_off();
  UnCalibrated =
      (eeprom_read_byte(&c_zero_tab[3]) - eeprom_read_byte(&c_zero_tab[0]));
  if (UnCalibrated != 0) {
    // if calibrated, both c_zero_tab values are identical! c_zero_tab[3] is not
    // used otherwise
    lcd_cursor_on();
  }
#endif
...
#else
  AutoCheck(
      0x01); // check, if selftest should be done, full selftest without MENU
#endif
#endif
#if FLASHEND > 0x1fff
  ...
  //  if(((PartFound == PART_NONE) || (PartFound == PART_RESISTOR) || (PartFound
  //  == PART_DIODE)) ) {
...
// If no part is found yet, check separate if is is a capacitor
  ...
  if (PartFound == PART_DIODE &&
      ...
      //            && (unsigned char)(ResistorList[0]+diodes.Anode[0])==2    //
      //            and resistor is between cathodes
      )
  // note: there also exist CUJTs (complementary UJTs); they seem to be (even)
  // rarer than UJTs, and are not supported for now
  {
    CheckUJT();
  }
...
#ifdef WITH_XTAL
  if (PartFound == PART_NONE ||
      ((PartFound == PART_CAPACITOR) && (cap.cpre_max == -12))) {
    // still not recognized anything? then check for ceramic resonator or
    // crystal
...
// All checks are done, output result to display
  ...
  if (PartFound == PART_UJT) {
    ...
    lcd_next_line(0);
    ResistorChecked[ntrans.e - TP_MIN + ntrans.c - TP_MIN - 1] =
        0; // forget last resistance measurement
    GetResistance(ntrans.c,
                  ntrans.e); // resistor value is in ResistorVal[resnum]
    DisplayValue(ResistorVal[ntrans.e - TP_MIN + ntrans.c - TP_MIN - 1], -1,
                 LCD_CHAR_OMEGA, 2);
    ...
    DisplayValue(ResistorVal[ntrans.e - TP_MIN + ntrans.c - TP_MIN - 1], -1,
                 LCD_CHAR_OMEGA, 2);
    lcd_data(',');
    DisplayValue(((RR680PL * (unsigned long)(ADCconfig.U_AVCC - ntrans.uBE)) /
                  ntrans.uBE) -
                     RRpinPL,
                 -1, LCD_CHAR_OMEGA, 3);
#endif /* SamplingADC */
  ...
  if (PartFound == PART_CAPACITOR) {
    ...
    show_cap(
        0); // show capacity in normal way and measure additional parameters
#else
    show_cap_simple(); // show capacity in normal way and measure additional
                       // parameters
#endif
    goto tt_end;
...
//        lcd_MEM_string(Diode);		//"Diode: "
      ...
      ReadCapacity(diodes.Cathode[0],
                   diodes.Anode[0]); // Capacity opposite flow direction
    ...
    showdiodecap:
      cap.cval =
          sampling_cap(diodes.Cathode[0], diodes.Anode[0], 0); // at low voltage
      lcd_next_line_wait(0); // next line, wait 5s and clear line 2
      DisplayValue(cap.cval, sampling_cap_pre, 'F', 2);
#ifdef PULLUP_DISABLE
      lcd_data('-');
      cap.cval = sampling_cap(diodes.Cathode[0], diodes.Anode[0],
                              1); // at high voltage
      if (cap.cval < 0)
        cap.cval = 0; // don't show negativ value
      DisplayValue(cap.cval, sampling_cap_pre, 'F', 2);
#if LCD_LINE_LENGTH > 16
...
#warning Capacity measurement from high to low not possible for diodes without PULLUP_DISABLE option!
      ...
      if (diodes.Anode[0] == diodes.Anode[1]) { // Common Anode
        ...
        UfAusgabe(0x01);
#ifdef SamplingADC
        goto showdiodecap; // double diodes are often varicap; measure
                           // capacitance of one of them
#else
        goto end3;
#endif
      }
      if (diodes.Cathode[0] == diodes.Cathode[1]) { // Common Cathode
        ...
        goto end3;
#endif
        //        else if ((diodes.Cathode[0] == diodes.Anode[1]) &&
        //        (diodes.Cathode[1] == diodes.Anode[0]))
      }
      if (diodes.Cathode[0] == diodes.Anode[1]) {
        // normaly two serial diodes are detected as three diodes, but if the
        // threshold is high
...
// ========================================
    ...
    if ((ptrans.count != 0) && (ntrans.count != 0)) {
      ...
      // If a protection diode is built on the same structur as the
      // NPN-Transistor,
      ...
      if ((ntrans.hfe < 500) && (ptrans.hfe >= 500))
        n_cpre = -16; // set NPN BE capacity to low value
      if ((ptrans.hfe < 500) && (ntrans.hfe >= 500))
        cap.cpre = -16; // set PNP BE capacity to low value

      if (((n_cpre == cap.cpre) && (cap.cval > n_cval)) ||
          (cap.cpre > n_cpre)) {
        // the capacity value or dimension of the PNP B-E is greater than the
        // NPN B-E
...
// if the Start key is still pressed, use the other Transistor
    ...
    lcd_big_icon(BJT_NPN |
                 LCD_UPPER_LEFT); // show the NPN Icon at lower left corner
    if (PartMode == PART_MODE_NPN) {
    ...
    if (vak_diode_nr < 5) {
// no side of the diode is connected to the base, this must be the protection
// diode
#ifdef WITH_GRAPHICS
      options = 0;
      ...
      lcd_update_icon_opt(
          bmp_vakdiode, options); // show the protection diode right to the Icon
    ...
    lcd_next_line(TEXT_RIGHT_TO_ICON + 1 -
                  LOW_H_SPACE); // position behind the icon+1, Line 3
    lcd_data('I');
...
#ifdef SHOW_ICE
    if (_trans->ice0 > 0) {
      lcd_next_line_wait(TEXT_RIGHT_TO_ICON - 1 -
                         LOW_H_SPACE); // position behind the icon, Line 4 &
                                       // wait and clear last line
...
// ========================================
    ...
    if (part_code == PART_MODE_JFET) {
      ...
      if (part_code == (PART_MODE_IGBT)) {
        ...
        if (fetidx == 1)
          lcd_update_icon(bmp_n_d_igbt);
        if (fetidx == 2)
          lcd_update_icon(bmp_p_e_igbt);
        if (fetidx == 3)
          lcd_update_icon(bmp_p_d_igbt);
      } else {
        lcd_MEM_string(mosfet_str); //"-MOS "
        lcd_big_icon(N_E_MOS | LCD_UPPER_LEFT);
        if (fetidx == 1)
          lcd_update_icon(bmp_n_d_mos);
        if (fetidx == 2)
          lcd_update_icon(bmp_p_e_mos);
        if (fetidx == 3)
          lcd_update_icon(bmp_p_d_mos);
      }
    }  /* end PART_MODE_JFET */
...
// MOSFET with protection diode; only with enhancement-FETs
      ...
      //  show diode symbol in right direction  (short form for less flash
      //  memory)
      if (an_cat) {
      ...
      if (_trans->c != diodes.Anode[vak_diode_nr])
        options |= OPT_VREVERSE;
      lcd_update_icon_opt(bmp_vakdiode,
                          options); // update Icon with protection diode
#endif
...
#ifdef WITH_GRAPHICS
    lcd_draw_trans_pins(
        -7, 16); // update of pin numbers must be done after diode update
    ...
    if ((PartMode & D_MODE) != D_MODE) { // enhancement-MOSFET
      ...
      lcd_next_line(
          TEXT_RIGHT_TO_ICON); // position text behind the icon, Line 3
      lcd_show_Cg();           // show Cg=xxxpF
#ifdef SHOW_R_DS
      lcd_show_rds(TEXT_RIGHT_TO_ICON -
                   1); // show RDS at column behind the icon -1
#endif
    } else {                                     /* depletion mode */
...
#define LINE_BELOW_ICON ((ICON_HEIGHT / 8) / ((FONT_HEIGHT + 7) / 8))
      ...
      if (_trans->ice0 < 4800) { // can't trust cutoff voltage if close to 5V
                                 // supply voltage, since then the transistor
                                 // may not have been cut off at all
...
// depletion
      ...
      lcd_next_line_wait(
          0); // line 2 or 3, if possible & wait and clear last line
#endif
...
#endif
        lcd_next_line_wait(
            0); // line 4, if possible & wait 5s and clear last line
...
#else
      // the following does the same as the above, but more cryptically (and in
      // fewer instructions)
      x = TP1 + (ii >> 1);
      y = TP3 - ii;
      z = TP2 + (ii > 0);
...
TyUfAusgabe:
...
#ifdef WITH_GRAPHICS
  lcd_set_cursor(1 * PAGES_PER_LINE,
                 TEXT_RIGHT_TO_ICON); // position behind the icon,line 2
#else
    lcd_line2(); // 2. row
#endif
  lcd_MEM_string(Uf_str); // "Uf="
  ...
  Display_mV(
      _trans->uBE,
      2); // needed instead of ntrans for PUT, but costs 4 bytes more flash...
#else
...
end2:
  ADC_DDR = (1 << TPRELAY) |
            TXD_MSK; // switch pin with reference to GND, release relay
  lcd_refresh();     // write the pixels to display, ST7920 only
  while (!(RST_PIN_REG & (1 << RST_PIN)))
...
// function search_vak_diode try to find a diode, which has no side connected to
// the transistor base
// returns 20, if no diode found
uint8_t search_vak_diode() {
  uint8_t ii;
  for (ii = 0; ii < NumOfDiodes; ii++) {
    if ((diodes.Anode[ii] == _trans->b) || (diodes.Cathode[ii] == _trans->b))
      continue;
    // no side of the diode is connected to the base, this must be the
    // protection diode
    if (diodes.Voltage[ii] > 1000)
      break; // Voltage is too high for protection diode
    return ii;
  }
  return 20;
}
