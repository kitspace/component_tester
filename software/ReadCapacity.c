...
// ReadCapacity tries to find the value of a capacitor by measuring the load
// time.
// first of all the capacitor is discharged.
// Then a series of up to 500 load pulses with 10ms duration each is done across
// the R_L (680Ohm)
// resistor.
// After each load pulse the voltage of the capacitor is measured without any
// load current.
// If voltage reaches a value of more than 300mV and is below 1.3V, the capacity
// can be
...
// The comparator can compare the voltage of any of the ADC input pins with the
// voltage
// of the internal reference (1.3V or 1.1V).
// After setting up the comparator and counter properly, the load of capacitor
// is started
// with connecting the positive pin with the R_H resistor (470kOhm) to VCC and
// immediately
// the counter is started. By counting the overflow Events of the 16bit counter
// and watching
// the counter event flag  the total load time of the capacitor until reaching
// the internal
...
void ReadCapacity(uint8_t HighPin, uint8_t LowPin) {
  ...
  if (HighPin == TestCapPin) {
    ...
    ADC_DDR =
        (1 << TestCapPin) | TXD_MSK; // switch capacitor-Pin to output (GND)
    wait_about20ms();
...
#ifdef AUTO_CAL
  pin_combination = ((HighPin - TP_MIN) * 3) + LowPin - TP_MIN -
                    1; // coded Pin combination for capacity zero offset
#endif

  HiPinR_L = pgm_read_byte(
      &PinRLRHADCtab[HighPin - TP_MIN]); // R_L mask for HighPin R_L load
  LoPinR_L = pgm_read_byte(
      &PinRLRHADCtab[LowPin - TP_MIN]); // R_L mask for LowPin R_L load
#if (((PIN_RL1 + 1) != PIN_RH1) || ((PIN_RL2 + 1) != PIN_RH2) ||               \
     ((PIN_RL3 + 1) != PIN_RH3))
  HiPinR_H = pgm_read_byte((&PinRLRHADCtab[3]) + HighPin -
                           TP_MIN); // R_H mask for HighPin R_H load
  ...
  for (ii = 0; ii < NumOfDiodes; ii++) {
    if ((diodes.Cathode[ii] == LowPin) && (diodes.Anode[ii] == HighPin) &&
        (diodes.Voltage[ii] < 1500)) {
#if DebugOut == 10
      lcd_data('D');  // debug
      wait_about2s(); /* debug delay */
#endif
  ...
  ADC_PORT = TXD_VAL; // switch ADC-Port to GND
  // The polarity of residual voltage of the capacitor depends on the
  // measurement
  ...
  // The voltage of the capacitor is build as difference between HighPin and
  // LowPin voltage.
  ADC_DDR = LoADC;   // switch Low-Pin to output (GND)
  R_DDR = LoPinR_L;  // switch R_L Port of LoPin to VCC
  R_PORT = LoPinR_L; // switch R_L Port of LoPin to VCC
  residual_voltage =
      ReadADC(HighPin) - ReadADC(LowPin); // voltage at HighPin before any load
...
// read voltage without current, is already charged enough?
    ...
    R_PORT = HiPinR_H; // R_H to 1 (VCC), to ensure that if nothing is
                       // connected, measured voltage will be Vcc, rather than
                       // float anywhere
    R_DDR = HiPinR_H; // shouldn't influence the measurements significantly,
                      // since the R is almost 1000 times larger, and the time
...
// wait 5ms and read voltage again, does the capacitor keep the voltage?
  ...
  if ((ovcnt16 == 0) && (cap_voltage1 > 1300)) {
    goto messe_mit_rh; // Voltage of more than 1300mV is reached in one pulse,
                       // too fast loaded
  }
// Capacity is more than about 50µF
#ifdef NO_CAP_HOLD_TIME
  ChargePin10ms(HiPinR_H,
                0); // switch HighPin with R_H 10ms auf GND, then currentless
  cap_voltage2 =
      ReadADC(HighPin) -
      residual_voltage; // read voltage again, is discharged only a little bit ?
#if DebugOut == 10
  lcd_data('U');
  lcd_data('3');
  lcd_data(':');
...
#endif
    //     if (ovcnt16 == 0 )  {
    //        goto messe_mit_rh;		// Voltage of more than 300mV is reached in
    //        one pulse, but not hold
    //     }
    goto keinC; // implausible, not yet the half voltage
  }
  cap.cval_uncorrected.dw = ovcnt16 + 1;
  cap.cval_uncorrected.dw *= GetRLmultip(
      cap_voltage1); // get factor to convert time to capacity from table
#else
  ...
  for (tmpint = 0; tmpint <= ovcnt16; tmpint++) {
    wait5ms();
    cap_voltage2 = ReadADC(HighPin) - residual_voltage; // read voltage again,
                                                        // is discharged only a
                                                        // little bit ?
    wdt_reset();
  }
  cap_voltage2 =
      cap_voltage1 - cap_voltage2; // lost voltage during load time wait
#if FLASHEND > 0x1fff
...
// more than 200mV is lost during load time
  ...
  cap.cval_uncorrected.dw = ovcnt16 + 1;
  // compute factor with load voltage + lost voltage during the voltage load
  // time
  cap.cval_uncorrected.dw *= GetRLmultip(
      cap_voltage1 +
      cap_voltage2);             // get factor to convert time to capacity from table
#endif
...
// shown
#if WITH_MENU
  cap.cval *= (1000 - (int8_t)eeprom_read_byte(
                          (uint8_t *)&big_cap_corr)); // correct with C_H_KORR
                                                      // with 0.1% resolution,
...
messe_mit_rh:
  ...
  ADC_DDR = (1 << TP1) | (1 << TP2) | (1 << TP3) |
            TXD_MSK;  // switch all Pins to output
  ...
  ACSR = (1 << ACBG) | (1 << ACI) |
         (1 << ACIC); // enable, 1.3V, no Interrupt, Connect to Timer1
  ...
  TI1_INT_FLAGS = (1 << ICF1) | (1 << OCF1B) | (1 << OCF1A) |
                  (1 << TOV1); // clear interrupt flags
#ifndef INHIBIT_SLEEP_MODE
  TIMSK1 =
      (1 << TOIE1) |
      (1
       << ICIE1); // enable Timer overflow interrupt and input capture interrupt
  unfinished = 1;
#endif
  ...
  if (PartFound == PART_FET) {
    ...
    TCCR1B = (0 << ICNC1) |
             (1 << CS10); // Start counter 1MHz or 8MHz without Noise Canceler
    ADC_DDR = (((1 << TP1) | (1 << TP2) | (1 << TP3) | TXD_MSK) &
               ~(1 << HighPin)); // release only HighPin ADC port
  } else {
    TCCR1B = (0 << ICNC1) |
             (1 << CS10); // start counter 1MHz or 8MHz without Noise Canceler
...
//******************************
  ...
  while (1) {
    ...
    if ((ii &
         (1 << TOV1))) { // counter overflow, 65.536 ms @ 1MHz, 8.192ms @ 8MHz
  ...
  TCCR1B = (0 << ICNC1) | (0 << ICES1) | (0 << CS10); // stop counter
  tmpint = ICR1; // get previous Input Capture Counter flag
  TIMSK1 = (0 << TOIE1) | (0 << ICIE1); // disable Timer overflow interrupt and
  ...
  load_diff =
      ReadADC(HighPin) + REF_C_KORR -
      ref_mv; // build difference of capacitor voltage to Reference Voltage
              //############################################################
  ...
  cap.cval_uncorrected.dw /=
      (F_CPU / 10000); // divide by 100 (@ 1MHz clock), 800 (@ 8MHz clock)
  ...
  if (cap.cpre == -12) {
    ...
    // note that we store this before subtracting cap_null, since for inductance
    // measurement that cap. also contributes
    if (cap.cval < 65536)
    ...
    if (HighPin == TP2)
      cap.cval +=
          TP2_CAP_OFFSET; // measurements with TP2 have 2pF less capacity
    if (cap.cval > C_NULL) {
...
checkDiodes:
  if ((NumOfDiodes > 0) && (PartFound != PART_FET)) {
    ...
    if ((cap.cpre > cap.cpre_max) ||
        ((cap.cpre == cap.cpre_max) && (cap.cval > cap.cval_max))) {
...
ISR(TIMER1_OVF_vect, ISR_BLOCK) {
  if ((!(TI1_INT_FLAGS & (1 << ICF1)) && (unfinished != 0)) ||
      ((TI1_INT_FLAGS & (1 << ICF1)) && (ICR1 < 250))) {
    // Capture Event not pending or (Capture Event pending and ICR1 < 250
    // the ICR1H is buffered and can not examined alone, we must read the ICR1L
    // first (with ICR1 access) !
...
ISR(TIMER1_CAPT_vect, ISR_BLOCK) {
  ...
  if ((TI1_INT_FLAGS &
    ...
    // ICR1H is buffered and can not examined alone, we must read the ICR1L
    // first (with ICR1 access) !
    if (ICR1 < 250) {
      //  Yes, Input Capture Counter is low, Overflow has occured before the
      //  capture event
      ovcnt16++; // count Overflow
    }
  }
