...
void DiodeSymbol_withPins(
    uint8_t nn); // Output the diode symbol with pins like 1->|-3
...
unsigned int
GetRLmultip(unsigned int cvolt); // get C-Multiplikator for voltage cvolt
void Scale_C_with_vcc(void); // scale capacity value for different VCC Voltages
void scale_intref_adc(
    void); // get scale factors for ReadADC with internal reference
// uint8_t value_out(unsigned long vval,uint8_t pp);    // output 4 digits with
// (pp-1) digits after point
void DisplayValue(
    signed long vval, int8_t Expo, unsigned char Unit,
    uint8_t Digits); // output Digits characters with exponent and unit
void DisplayValue16(
    uint16_t vval, int8_t Expo, unsigned char Unit,
    uint8_t Digits); // output Digits characters with exponent and unit
...
void sleep_5ms(uint8_t xxx); // set processor to sleep state for xxx times 5ms,
                             // xxx>200  (xxx-200)*100
void PinLayout(char pin1, char pin2,
               char pin3); // show pin layout with EBC= or 123=
void PinLayoutLine(char pin1, char pin2,
                   char pin3);     // show pin layout with Pin 1=...
...
uint8_t ShortedProbes(uint8_t p1,
                      uint8_t p2); // check if two probes are shorted
...
uint8_t
wait_for_key_ms(int max_time);    // wait max_time ms units for key is pressed
...
void EE_check_init(
    void); // check calibration values and init the values if unset
...
void set_big_cap_corr();          // set the correction value for big capacitor
                                  // measurement
void message_key_released(const unsigned char XX_str[]); // clear display, show
                                                         // message XX_str and
                                                         // wait for released
                                                         // key
...
void show_Cap13(
    void); // show Capacitor value at TP1:TP3, measuring continuously
void show_resis(
    uint8_t pin1, uint8_t pin2,
    uint8_t how); // show resistor (and inductance) between pin1 and pin2
void show_cap(uint8_t how); // show capacity and measure additional values
void show_cap_simple(
    void); // show capacity and measure additional values without [C]
void ReadBigCap(uint8_t HighPin,
                uint8_t LowPin); // capacity measurement only for big capacitors
void CheckRotaryEncoder(void);   // check rotary puls generator
void check_rotary(void);         // check state of rotary switch	for menu
                                 // function
void message2line(uint8_t number); // write menue message number to lcd line
void switch_frequency(
    uint8_t freq_num);     // switch frequency and write actual frequency to LCD
...
uint8_t
Pwr_mode_check(uint8_t ll);  // give next counter state depending on DC_Pwr_mode
...
uint8_t search_vak_diode(void); // search the diode number of the diode
                                // connected to emitter and collector
void mVOut(uint16_t vv);        // mV output for debug
void setFScaler();              // ask for new frequency scaler
uint16_t
unsigned_diff(uint16_t v1,
              uint16_t v2); // build difference v1-v2, but 0 instead of negative
...
uint16_t samplingADC( // code is in sampling_ADC.S
    ...
    void *ptr,     // output pointer (note: ptr[0] will contain incorrect data;
                   // ptr[1]...ptr[n-1] will be valid) R22:R23
    uint8_t n,     // number of samples (n=0 for 256), R20
    uint8_t Rport_1, uint8_t Rddr_1, uint8_t Rport_0, uint8_t Rddr_0);
// ptr can point to an array of either 8 or 16 bit data (with or without
// smplADC_8bit, respectively)
...
// The measurement cycle consist of three phases, henceforth called idle, pulse,
...
// If the samplingADC_direct option is NOT used, during pulse Rddr_0 and Rport_1
// apply (duration of pulse is between 1 and 16 CPU cycles, controlled by lower
// bits of 'what' parameter).
//  (note though that currently the non-direct mode is only used for step
//  response measurements, with Rport_1=Port_0, so effectively no pulse is
//  applied)
// If the samplingADC_direct option is used, a 1 CPU-cycle pulse is applied via
// de ADC pin corresponding to a 1 bit in Rddr_0.
...
#ifdef WITH_XTAL
void sampling_test_xtal(void); // test presence of resonator or crystal between
                               // pin 1 and 3 (hardcoded)
uint8_t sampling_measure_xtal(
    void); // measure resonator or crystal between pin 1 and 3 (hardcoded),
...
// version to be called when using samlingADC_freq option, to pass the desired
// frequency:
uint16_t samplingADC_freqgen(uint16_t, void *, uint8_t, uint8_t, uint8_t,
                             uint8_t, uint8_t, uint16_t freq);
// version to be called when using samlingADC_sck option, to also pass the
// desired short circuit duration:
uint16_t samplingADC_freqgen_sck(uint16_t, void *, uint8_t, uint8_t, uint8_t,
                                 uint8_t, uint8_t, uint16_t freq,
                                 uint8_t shortcircuitduration);
#endif
#endif
