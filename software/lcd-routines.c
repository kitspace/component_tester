...
// for the ST7920 you can select a accurate vertical pixel positioning of line
// with:
...
/* **************************************************************************************
 */
/* Set the character position to x,y , where x specifies the character number in
 * a text line. */
/* The y position is the page address (8 line units). */
/* For most controllers the y position must be increased by (Height + 7) / 8 for
 * the next text line */
void lcd_set_cursor(uint8_t y, uint8_t x) {
  ...
  // LCD_ST7565_H_OFFSET specifies the offset of the 128 pixel of the display
  // window.
  _xpos = (x * FONT_WIDTH);
#else
...
void lcd_restore_position(void) {
  lcd_set_cursor((uint8_t)last_text_line * PAGES_PER_LINE,
                 (uint8_t)(last_text_column));
}
#endif
...
// with offset  64, 0x40 the icon is written to the left lower quarter of the
// screen
// with offset 128, 0x80 the icon is written to the right upper quarter of the
// screen
// with offset 192, 0xC0 the icon is written to the left upper quarter of the
// screen
...
#define DIODE_WIDTH                                                            \
  5 /* diode is 8 pixel width, but TP overlap the diode images with 3 pixels   \
       */
#if SCREEN_HEIGHT > 64
...
#define HALF_SCREEN_HEIGHT (SCREEN_HEIGHT / 2)
  ...
  if ((temp1 & 0x40) == 0)
    icon_xx += (SCREEN_WIDTH / (ONE_B / 4)); // right side
  icon_yy = 0;
  if ((temp1 & 0x80) == 0) {
    icon_yy = HALF_SCREEN_HEIGHT; // lower half of display
    // shift lower icon position to the right
    icon_xx += (SCREEN_WIDTH / (ONE_B / 4)) - ICON_WIDTH - TP_WIDTH - TP_WIDTH -
               DIODE_WIDTH;
  }

  lcd_set_pixels(pfont, icon_xx, icon_yy, 0, ICON_WIDTH,
                 ICON_HEIGHT); // write the pixel data of the Icon
} /* end lcd_big_icon() */

/* *******************************************************************************
 */
// update a icon
// The left upper corner of the last loaded Icon is positioned to icon_xx,
// icon_yy
// You can change parts of this Icon with a lcd_update_icon call.
// The only parameter is a pointer to a array of bytes.
// The first two bytes specify the location of the change bitmap relative to the
// upper left corner
// (the start position) of the last loaded Icon ( call of
// lcd_big_icon(ICON_NUMBER);).
...
void lcd_update_icon_opt(const unsigned char *ubitmap, unsigned char options) {
  ...
  xx = icon_xx + pgm_read_byte(&ubitmap[0]); // first byte = offset x to left
                                             // (upper) corner of the Icon
  yy = icon_yy + pgm_read_byte(&ubitmap[1]); // second byte = offset y to (left)
                                             // upper corner of the Icon
  ww = pgm_read_byte(&ubitmap[2]);           // the width of the bitfield
  hh = pgm_read_byte(&ubitmap[3]); // the height of the bitfield (must be
                                   // multiple of 8 for ST7565)
  lcd_set_pixels(
      pfont, xx, yy, options, ww,
      hh); // update the icon at the specified position with specified size
...
void lcd_data(unsigned char temp1) {
  ...
  uint8_t *pfont;
  pfont = (uint8_t *)font +
          (FONT_WIDTH * PAGES_PER_LINE * temp1); // first byte of character data
  // for other controllers like ST7565 the _page specifies the page of the
  // controller (8 lines)
  lcd_set_pixels(pfont, _xpos, _page * 8, 0, (unsigned char)FONT_WIDTH,
                 (unsigned char)(PAGES_PER_LINE * 8));
...
void lcd_command(unsigned char temp1) {
  _lcd_hw_write(0x00, temp1);
     ...
     (LCD_ST_TYPE == 7108) || (LCD_ST_TYPE == 7920) ||                         \
     (LCD_ST_TYPE == 8812) || (LCD_ST_TYPE == 8814) ||                         \
     (LCD_ST_TYPE == 7735) || (LCD_ST_TYPE == 9163) || (LCD_ST_TYPE == 9341))
  ;
#else
...
// for 3LINE mode the RS is chip enable, RS is send as first serial bit
  ...
  wait_about100ms();     // Set LCD for 100 ms into RESET
#elif (LCD_INTERFACE_MODE ==                                                   \
       MODE_7920_SERIAL)                    /* ! MODE_SPI || MODE_3LINE || MODE_I2C */
...
#elif (LCD_INTERFACE_MODE == MODE_PARALLEL) /* ! MODE_SPI || MODE_3LINE ||     \
                                               MODE_I2C || MODE_7920_SERIAL */
/* ! MODE_SPI || MODE_3LINE || MODE_I2C || MODE_7920_SERIAL */
  ...
  lcd_command(CMD_SET_POWER_CONTROL |
              7); // 0x28 BOOSTER ON | V_REGULATOR ON | V_FOLLOWER ON
  ...
  lcd_command(eeprom_read_byte(&EE_Volume_Value) &
              0x3f); // set volume value of EEprom

//   lcd_command(CMD_SET_STATIC_OFF);		// 0xac , set Static indicator
//   off
//   lcd_command(CMD_SET_STATIC_REG | 0);	// 0x00 , set Static indicator no
//   flashing

//##   lcd_command(CMD_RMW);			// 0xe0 , set Read Modify Write
//mode
  ...
  lcd_command(
      CMD_SET_DIVIDE_RATIO); // set display clock div (0xd5), 0x70 is default
  ...
  //   lcd_command(CMD_SET_MEMORY_ADDR_MODE);		// set memory addressing mode
  //   (0x20)
  //   lcd_command(0x00);		// horizontal mode

  //   lcd_command(CMD_SET_COM_Pins_CONFIG);		// sequential COM pin
  //   configuration (0xda)
  //   lcd_command(0x12);		// disable COM Left/Right remap, alternative COM
  //   pin

  lcd_command(CMD_SET_VOLUME_FIRST);               // set contrast
  lcd_command(eeprom_read_byte(&EE_Volume_Value)); // set volume value of EEprom

//   lcd_command(CMD_SET_PreCharge_PERIOD);		// set precharge periodi
//   (0xd9), 0x22 is reset value
//   lcd_command(0xf1);

//   lcd_command(CMD_SET_Vcomh_DESELECT_LEVEL);		// set Vcomh deselect level
//   (0xdb), 0x20 is reset value
//   lcd_command(0x40);
//##   lcd_command(CMD_RMW);			// 0xe0 , set Read Modify Write
//mode
#endif
  ...
  wait_about100ms();
  lcd_command(CMD_SET_EXTENDED_INSTRUCTION); // 0x21 switch power on, extended
                                             // instruction
  lcd_command(ECMD_SET_HV_STAGES | (LCD_ST7565_RESISTOR_RATIO /
                                    2)); // 0x08|0x03 set charge pump stages (3)
  ...
  lcd_command(ECMD_SET_CONTRAST |
              (eeprom_read_byte(&EE_Volume_Value) &
               0x7f)); // set volume value of EEprom (0x4e=78)
  lcd_command(
      CMD_SET_NORMAL_INSTRUCTION); // switch power on, normal instruction
  lcd_clear();                     // clear display
  //   lcd_command(CMD_SET_DISPLAY_NORMAL);			// set display to normal
  //   mode
  ...
  lcd_command(CMD_SET_START_LINE |
              (LCD_ST7565_Y_START & 0x3f)); // Set the Start line 0
...
/* -------------------------------------------------------------------------- */
  ...
  lcd_command(CMD_SET_ENTRY_MODE |
              MODE_RIGHT_MOVE); // increase address counter after read/write
...
/* -------------------------------------------------------------------------- */
#elif (LCD_ST_TYPE ==                                                          \
       7108) /* !(LCD_ST_TYPE == 7565 | 1306 | 8812 | 8814 | 7920) */
  wait_about50ms();
...
/* -------------------------------------------------------------------------- */
       ...
       (LCD_ST_TYPE ==                                                         \
        9163)) /* !(LCD_ST_TYPE == 7565 | 1306 | 8812 | 8814 | 7920 | 7108) */
  ...
  lcd_write_data(
      0x04); // Gamma curve 3; 0x01= curve 1, 0x02= curve2, 0x08= curve 4
  ...
  lcd_write_data(0x3c); // MV=exchange xy, ML=Vertical refresh, RGB=BGR
                        // color,MH=Refresh right to left
#else
  lcd_write_data(
      0x1c); // ML=Vertical refresh, RGB=BGR color,MH=Refresh right to left
#endif
  ...
  wait_about50ms();
/* -------------------------------------------------------------------------- */
#elif (LCD_ST_TYPE == 9341) /* !(LCD_ST_TYPE == 7565 | 1306 | 8812 | 8814 |    \
                               7920 | 7108 | 7735 | 9163) */
  /* --------------------------------------------------------------------------
   */
  /* init sequence for ILI9341 is taken from the ComponentTester-trendy code of
   * Markus R. */
  ...
  lcd_write_word(0x5c4c); /* VCOMH = 5.000V, could be used to adjust contrast,
                             VCOML = -0.600V */
  ...
  lcd_command(
      CMD_FRAME_RATE_CONTROL); /* frame control for normal display mode */
...
#define MODE_BDS 0x00
  ...
  lcd_command(CMD_SetIFOptions | MODE_8BIT |
              0x0a); // 0x2a|0x3a 8Bit / 2 rows /RE=1
  lcd_command(CMD_SetIFOptions | MODE_8BIT |
              0x0a);                       // 0x2a|0x3a 8Bit / 2 rows /RE=1
  ...
  lcd_command(CMD_SetIFOptions | MODE_8BIT |
              0x09);                // 0x29 4Bit / 2 rows / 5x7 / Instr. table 1
...
#define DEFAULT_RATIO 0x0e
  ...
  lcd_command(
      CMD1_PowerControl |
      ((contrast >> 4) & 0x07)); // 0x5x booster on,off / set contrast C5:C4
#ifdef LCD_ST7565_RESISTOR_RATIO
  lcd_command(CMD1_FollowerControl | 0x08 |
              LCD_ST7565_RESISTOR_RATIO); // 0x6e Follower on / Rab2:0 = RATIO
#else
  lcd_command(CMD1_FollowerControl |
              DEFAULT_RATIO); // 0x6e Follower on / Rab2:0 = 6
#endif
  ...
  lcd_command(CMD_SetIFOptions | MODE_8BIT |
              0x08); // 4Bit / 2 rows / 5x7 / Instruction Table 0
  lcd_clear();
  ...
  lcd_command(CMD_SetIFOptions | MODE_8BIT | 0x40 |
              0x08);                           // enter extended instruction
  lcd_command(CMD_SetDisplayAndCursor | 0x01); // enable 4-line mode
#endif
  lcd_command(CMD_SetIFOptions | MODE_8BIT |
              0x0A); // 4Bit / 2 rows / 5x7 / table3 / Add for OLED
  lcd_command(
      CMD_SetDisplayAndCursor); // Display off / no Blinking  / Add for OLED
  lcd_command(CMD_SetEntryMode | 0x02); // increment / no Scroll
  lcd_command(CMD_CURSOR_HOME);         // Home Command  // Add for OLED
  lcd_clear();
  wait_about5ms();
  lcd_command(CMD_SetDisplayAndCursor |
              0x04); // Display on / Cursor off / no Blinking
#endif
                                        // - - - - - - - - - - - - - - - -
...
// if kyrillish LCD-Characterset is defined, load  Omega- and µ-Character
  ...
  LCDLoadCustomChar(LCD_CHAR_RESIS3); // load Resistor symbol as
                                      // Custom-Character
...
void lcd_clear(void) {
  ...
  for (p = 0; p < (SCREEN_HEIGHT / 8); p++) {
    ...
    lcd_command(CMD_SET_PAGE |
                (0x07 & p)); // set page 0 to 7, vertical position = p*8
...
void lcd_clear_line(void) {
  // writes spaces up to LCD_LINE_LENGTH to LCD-Display, Cursor can be
  // positioned to any column
...
/* The first parameter pbitmap is a pointer to a array of bytes in the program
 * memory */
/* the first two bytes in pbitmap specifies the size (width + height) of the
 * bitfield */
/* the following data are written to the display to position x,y with options */
/* *******************************************************************************
 */
#ifdef WITH_GRAPHICS
void lcd_pgm_bitmap(const unsigned char *pbitmap, unsigned char x,
                    unsigned char y, unsigned char options) {
  uint8_t dx;                                     // width
  uint8_t dy;                                     // height
...
/* pdata is a pointer to the byte array in the program memory holding the pixel
 * data. */
/* x,y specifies the target position of this data at the display. */
/* The parameter options can specify a inverted position in x or/and y
 * direction.  */
...
/* *******************************************************************************
 */
void lcd_set_pixels(const unsigned char *pdata, unsigned char x,
                    unsigned char y, unsigned char options, unsigned char width,
                    unsigned char height) {
/* -------------------------------------------------------------------------------
 */
#if ((LCD_ST_TYPE == 7565) || (LCD_ST_TYPE == 1306) ||                         \
     (LCD_ST_TYPE == 7108) || (LCD_ST_TYPE == 8812) || (LCD_ST_TYPE == 8814))
 ...
 */
#elif (LCD_ST_TYPE == 7920)
  // lcd_set_pixels(const uchar *pdata, uchar x, uchar y, uchar options, uchar
  // width, uchar height)
...
// clear bit in the pixel image
        ...
        lcd_bit_mem[ymem][((SCREEN_WIDTH / 8) - 1) - xx] &=
            pgm_read_byte(clear_msk + xxbit);
#else
...
// set bit in the pixel image
  ...
  /* -------------------------------------------------------------------------------
   */
  // void lcd_set_pixels(const unsigned char *pdata, unsigned char x, unsigned
  // char y,
  // unsigned char options, unsigned char width, unsigned char height)
  // support for color display
  if (((x + width) > (SCREEN_WIDTH / (ONE_B / 8))) ||
      ((y + height) > (SCREEN_HEIGHT / (ONE_B / 8))))
    return;
...
#if (LCD_ST7565_V_FLIP == 1)
      lcd_write_word(((SCREEN_HEIGHT - 1) - ONE_B) - page +
                     LCD_ST7565_V_OFFSET);
      lcd_write_word(((SCREEN_HEIGHT - 1) - ONE_B) - page +
                     LCD_ST7565_V_OFFSET + (ONE_B - 1));
#else
...
void lcd_show_rds(uint8_t xpos) {
  if ((PartMode & 0x0f) == PART_MODE_MOS) { /* kein JFET or IGBT */
    ...
    DisplayValue16(_trans->uBE, -1, LCD_CHAR_OMEGA,
...
/* usually the refresh is done after the display data are finished before
 * waiting */
void lcd_refresh(void) {
  ...
  for (yy = 0; yy < SCREEN_HEIGHT; yy++) {
    ...
    if (yyo < 32) {
      ...
      lcd_command(CMD_SET_GDRAM_ADDRESS |
                  0); /* horizontal address starts with 0 */
    } else {
      // the second half of display is located at GDRAM address 128..255
      // ((8..15) x 16 pixel)
      lcd_command(CMD_SET_GDRAM_ADDRESS |
                  (yyo - 32)); /* set vertical start address */
      lcd_command(
          CMD_SET_GDRAM_ADDRESS |
          (SCREEN_WIDTH / 16)); /* horizontal address starts with 0 (128)*/
    }
    for (xx = 0; xx < (SCREEN_WIDTH / 8); xx++) {
      lcd_write_data(
          lcd_bit_mem[yy][xx]); // write 16 x 8 bits to GDRAM (= 128 pixel)
    }
  } /* end for yy */
  wdt_reset();
}
